...
import {
  ...
  transformUserResponseANDX,
} from "./cakeTransformers";
import {
  transformBetResponseBNB,
  transformUserResponseBNB,
} from "./bnbTransformers";
...
const convertBigInt = (value: string | null): null | bigint =>
...
export const transformBetResponse = (tokenSymbol) =>
  tokenSymbol === "ANDX" ? transformBetResponseANDX : transformBetResponseBNB;

export const transformUserResponse = (tokenSymbol) =>
  tokenSymbol === "ANDX" ? transformUserResponseANDX : transformUserResponseBNB;
...
const getTotalWonMarket = (market, tokenSymbol) => {
  const total = market[`total${tokenSymbol}`]
    ? parseFloat(market[`total${tokenSymbol}`])
    : 0;
  const totalTreasury = market[`total${tokenSymbol}Treasury`]
    ? parseFloat(market[`total${tokenSymbol}Treasury`])
    : 0;
...
export const getTotalWon = async (): Promise<{
  ...
  const [{ market: BNBMarket, market: ANDXMarket }] = await Promise.all([
    request(
      GRAPH_API_PREDICTION_BNB,
      gql`
  ...
  const totalWonANDX = getTotalWonMarket(ANDXMarket, "ANDX");

  return { totalWonBNB, totalWonANDX };
};
...
export const getBetHistory = async (
  ...
  api: string,
  tokenSymbol: string
): Promise<Array<BetResponseBNB | BetResponseANDX>> => {
...
export type MarketData = Pick<
  PredictionsState,
  "status" | "currentEpoch" | "intervalSeconds" | "minBetAmount"
>;
export const getPredictionData = async (
  address: Address
): Promise<MarketData> => {
  const [currentEpoch, intervalSeconds, minBetAmount, paused] =
...
export const fetchUserRounds = async (
  ...
  try {
    const [rounds, ledgers] = await contract.read.getUserRounds([
      account,
      BigInt(cursor),
      BigInt(size),
    ]);
