...
import { ANDX, USDC, USDT, STABLE_COIN } from "@pancakeswap/tokens";
...
import fetchDerivedPriceData, {
  getTokenBestTvlProtocol,
} from "./fetch/fetchDerivedPriceData";
import {
...
export function useSingleTokenSwapInfo(
  ...
  const token0Address = useMemo(
    () => getTokenAddress(chainId, inputCurrencyId),
    [chainId, inputCurrencyId]
  );
  const token1Address = useMemo(
    () => getTokenAddress(chainId, outputCurrencyId),
    [chainId, outputCurrencyId]
  );

  const amount = useMemo(
    () => tryParseAmount("1", inputCurrency ?? undefined),
    [inputCurrency]
  );
...
export const useFetchPairPricesV3 = ({
  ...
  const {
    ...
    {
      enabled: Boolean(
        protocol0 && protocol1 && token0Address && chainId && token1Address
      ),
  ...
  const normalizedDerivedPairDataWithCurrentSwapPrice = useMemo(
    () =>
      ...
      normalizedDerivedPairData?.length > 0 &&
      hasSwapPrice
        ? [
            ...normalizedDerivedPairData,
