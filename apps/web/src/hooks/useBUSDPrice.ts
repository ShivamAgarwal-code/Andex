...
import { ANDX, STABLE_COIN } from "@pancakeswap/tokens";
...
export function useStablecoinPrice(
  ...
  const { enabled, hideIfPriceImpactTooHigh } = {
    ...DEFAULT_CONFIG,
    ...config,
  };
  ...
  const stableCoin =
    chainId && chainId in ChainId ? STABLE_COIN[chainId as ChainId] : undefined;
  const isCake =
    chainId &&
    currency &&
    ANDX[chainId] &&
    currency.wrapped.equals(ANDX[chainId]);

  const isStableCoin =
    currency && stableCoin && currency.wrapped.equals(stableCoin);
  ...
  const enableLlama = shouldEnabled;
  ...
  const { trade } = useBestAMMTrade({
    ...
    maxSplits: 0,
    enabled: Boolean(
      enableLlama ? !isLoading && !priceFromLlama : shouldEnabled
    ),
  ...
  const price = useMemo(() => {
    ...
    if (isCake && cakePrice) {
      return new Price(
        ...
        getFullDecimalMultiplier(stableCoin.decimals)
          .times(cakePrice.toFixed(stableCoin.decimals))
          .toString()
      );
    }
    ...
    if (trade) {
      const { inputAmount, outputAmount } =
        trade as unknown as SmartRouterTrade<TradeType>;
      ...
      if (hideIfPriceImpactTooHigh) {
        ...
        if (
          ...
          warningSeverity(priceImpactWithoutFee) > 2
        ) {
          return undefined;
        }
      }
