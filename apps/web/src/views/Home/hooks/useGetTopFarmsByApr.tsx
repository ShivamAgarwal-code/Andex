...
const useGetTopFarmsByApr = (isIntersecting: boolean) => {
  ...
  const { data: farms, regularCakePerBlock } = {
    data: [] as any,
    regularCakePerBlock: 10,
  };
  const { data: farmsV3, isLoading } = { data: {} as any, isLoading: false };
  ...
  useEffect(() => {
    if (fetchStatus === "success" && farms?.length > 0 && !isLoading) {
      ...
      const sortedByApr = orderBy(
        ...
        (farm) =>
          farm.apr !== null ? farm.apr + farm.lpRewardsApr : farm.lpRewardsApr,
        "desc"
      );
  ...
  return {
    topFarms,
    fetched: fetchStatus === "success" && !isFetching,
    chainId,
  };
};
