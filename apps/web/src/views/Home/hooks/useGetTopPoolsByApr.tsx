...
const useGetTopPoolsByApr = (isIntersecting: boolean, chainId: number) => {
  ...
  const [topPools, setTopPools] = useState<
    (Pool.DeserializedPool<Token> | any)[]
  ...
  useEffect(() => {
    const [cakePools, otherPools] = partition(
      pools,
      (pool) => pool.sousId === 0
    );
    const masterCakePool = cakePools.filter(
      (cakePool) => cakePool.vaultKey === VaultKey.CakeVault
    );
    const getTopPoolsByApr = (
      activePools: (Pool.DeserializedPool<Token> | any)[]
    ) => {
      const sortedByApr = orderBy(
        activePools,
        (pool: Pool.DeserializedPool<Token>) => pool.apr || 0,
        "desc"
      );
