...
/**
 * User (Planet Finance) built a contract on top of our original manual ANDX pool,
 ...
 * https://twitter.com/Andex/status/1523913527626702849
...
const CakeDataRow = () => {
  ...
  const {
    ...
    ["cakeDataRow"],
    async () => {
      const [totalSupply, burned, totalLockedAmount] = await publicClient({
      ...
      return {
        cakeSupply:
          totalSupply && burned ? +formatBigInt(totalSupply - totalBurned) : 0,
  ...
  return (
    <StyledWrapper mb={isMobile ? "30px" : "50px"}>
      ...
      <StyledColumn>
        ...
        {emissionsPerBlock ? (
          <Heading color="secondary" scale="lg">
            {t("%cakeEmissions%/block", {
              cakeEmissions: formatNumber(emissionsPerBlock),
            })}
          </Heading>
        ) : (
          <Skeleton height={24} width={126} my="4px" />
