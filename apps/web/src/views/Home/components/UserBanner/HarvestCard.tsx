...
// import useFarmsWithBalance, {
//   FarmWithBalance,
// } from "views/Home/hooks/useFarmsWithBalance";
...
const HarvestCard: React.FC<React.PropsWithChildren<HarvestCardProps>> = ({
  onHarvestStart,
  onHarvestEnd,
}) => {
  ...
  // const { farmsWithStakedBalance, earningsSum: farmEarningsSum } =
  //   useFarmsWithBalance();

  const cakePriceBusd = useCakePrice();
  const { isMobile } = useMatchBreakpoints();
  const gasPrice = useGasPrice();
  const earningsBusd = new BigNumber(0).multipliedBy(cakePriceBusd);
  const numTotalToCollect = 0;
  const numFarmsToCollect = 0;
  const hasCakePoolToCollect = numTotalToCollect - numFarmsToCollect > 0;

  const earningsText = getEarningsText(
    numFarmsToCollect,
    hasCakePoolToCollect,
    earningsBusd,
    t
  );
  ...
  //           {t('Your %symbol% earnings have been sent to your wallet!', { symbol: 'ANDX' })}
