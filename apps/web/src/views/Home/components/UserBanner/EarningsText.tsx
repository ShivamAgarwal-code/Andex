...
export const getEarningsText = (
  ...
  if (numFarmsToCollect > 0 && hasCakePoolToCollect) {
    if (numFarmsToCollect > 1) {
      earningsText = t(
        "%earningsBusd% to collect from %count% farms and ANDX pool",
        data
      );
    } else {
      earningsText = t(
        "%earningsBusd% to collect from %count% farm and ANDX pool",
        data
      );
    }
    ...
    earningsText = t("%earningsBusd% to collect from ANDX pool", data);
  }
