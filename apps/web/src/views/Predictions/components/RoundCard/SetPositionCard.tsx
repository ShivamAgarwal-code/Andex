...
const TOKEN_BALANCE_CONFIG = {
  ...
  ANDX: useBSCCakeBalance,
} as const;

const SetPositionCard: React.FC<
  React.PropsWithChildren<SetPositionCardProps>
> = ({ position, togglePosition, epoch, onBack, onSuccess }) => {
  ...
  const useTokenBalance = useMemo(() => {
    return TOKEN_BALANCE_CONFIG[
      token.symbol as keyof typeof TOKEN_BALANCE_CONFIG
    ];
  }, [token.symbol]);

  const { setLastUpdated, allowance } = useCakeApprovalStatus(
    token.symbol === "ANDX" ? predictionsAddress : null
  );
  ...
  const doesCakeApprovePrediction =
    token.symbol === "BNB" || allowance.gte(valueAsBn.toString());
  ...
  const handlePercentChange = useCallback(
    (sliderPercent: number) => {
      if (sliderPercent > 0) {
        const maxValueAsFn = new BN(maxBalance.toString());
        const hundredAsFn = new BN(100);
        const sliderPercentAsFn = new BN(sliderPercent.toFixed(18)).div(
          hundredAsFn
        );
  ...
  const handleEnterPosition = async () => {
    ...
    const args =
      token.symbol === "ANDX" ? [epoch, valueAsBn.toString()] : [epoch];

    const receipt = await fetchWithCatchTxError(() => {
      return callWithGasPrice(
        predictionsContract as any,
        betMethod,
        args,
        callOptions
      );
  ...
  useEffect(() => {
    ...
    if (!hasSufficientBalance) {
      ...
      setErrorMessage(
        t("A minimum amount of %num% %token% is required", {
          num: formatBigInt(minBetAmount),
          token: token.symbol,
        })
      );
    } else {
          ...
          {account ? (
            doesCakeApprovePrediction ? (
              <Button
                ...
                endIcon={
                  isTxPending ? (
                    <AutoRenewIcon color="currentColor" spin />
                  ) : null
                }
              >
              ...
              <Button
                ...
                isLoading={pendingTx}
                endIcon={
                  pendingTx ? <AutoRenewIcon color="currentColor" spin /> : null
                }
              >
                {t("Enable")}
              </Button>
