...
const PredictionConfigProviders = ({ children }) => {
  ...
  const [selectedToken, setConfig] = useState(() => {
    const initToken = localStorage?.getItem(
      PREDICTION_TOKEN_KEY
    ) as PredictionSupportedSymbol;

    if (
      [PredictionSupportedSymbol.BNB, PredictionSupportedSymbol.ANDX].includes(
        initToken
      )
    ) {
      return initToken;
    ...
    return PredictionSupportedSymbol.ANDX;
  });
