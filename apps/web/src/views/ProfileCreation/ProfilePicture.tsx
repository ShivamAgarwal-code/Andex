    ...
    const fetchUserPancakeCollectibles = async () => {
      try {
        ...
        if (nftsByCollection.length > 0) {
          ...
          const collectionRoles = (await publicClient({
          ...
          setUserProfileCreationNfts(
            nfts.filter(
              (nft) =>
                collectionRoles[nftsByCollection.indexOf(nft.collectionAddress)]
            )
          );
        } else {
    ...
    return (
      <>
        ...
        <Text as="p" mb="24px">
          ...
          <Link
            to={`${nftsBaseUrl}/profile/pancake-collectibles`}
          ...
          <NftWrapper>
            {userProfileCreationNfts?.length ? (
              userProfileCreationNfts
                .filter(
                  (walletNft) => walletNft.location === NftLocation.WALLET
                )
                .map((walletNft) => {
                  return (
                    <SelectionCard
                      ...
                      onChange={(value: string) =>
                        actions.setSelectedNft(
                          value,
                          walletNft.collectionAddress
                        )
                      }
                    >
