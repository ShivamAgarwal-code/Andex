...
const UserName: React.FC<React.PropsWithChildren> = () => {
  ...
  const { teamId, selectedNft, userName, actions, allowance } =
    useProfileCreation();
  ...
  const hasMinimumCakeRequired =
    fetchStatus === FetchStatus.Fetched && cakeBalance >= REGISTER_COST;
      ...
      try {
        ...
        if (!debouncedUsernameToCheck) {
          ...
          const res = await fetchWithTimeout(
            `${API_PROFILE}/api/users/valid/${debouncedUsernameToCheck}`,
            {
              method: "get",
              signal: abortSignal,
              timeout: 30000,
      ...
      {!hasMinimumCakeRequired && (
        <Text color="failure" mt="16px">
          {t("A minimum of %num% ANDX is required", {
            num: formatUnits(REGISTER_COST, 18),
          })}
        </Text>
      )}
    </>
