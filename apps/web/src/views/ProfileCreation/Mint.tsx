...
const Mint: React.FC<React.PropsWithChildren> = () => {
  ...
  const hasMinimumCakeRequired =
    fetchStatus === FetchStatus.Fetched && cakeBalance >= MINT_COST;
  ...
  const {
    ...
    onConfirm: () => {
      return callWithGasPrice(bunnyFactoryContract, "mintNFT", [
        BigInt(selectedBunnyId),
      ]);
    },
    onApproveSuccess: () => {
  ...
  return (
    <>
      ...
      <Text as="p">
        {t("Every profile starts by making a “starter” collectible (NFT).")}
      </Text>
      <Text as="p">
        {t("This starter will also become your first profile picture.")}
      </Text>
      <Text as="p" mb="24px">
        {t(
