...
const StartPage: React.FC<React.PropsWithChildren<StartPageProps>> = ({
  ...
  const minimumCakeRequired = profile?.isActive
    ? numberCakeToUpdate
    : numberCakeToReactivate;
  const hasMinimumCakeRequired =
    fetchStatus === FetchStatus.Fetched && cakeBalance >= minimumCakeRequired;
  const { handleEnable, pendingEnableTx } = useCakeEnable(
    new BigNumber(minimumCakeRequired.toString())
  );
  ...
  /**
   * Check if the wallet has the required ANDX allowance to change their profile pic or reactivate
      ...
      {profile.isActive ? (
        <>
          <Message variant="warning" my="16px">
            <MessageText>
              {t(
                "Before editing your profile, please make sure you've claimed all the unspent ANDX from previous IFOs!"
              )}
            </MessageText>
          </Message>
          {showCakeRequireFlow ? (
            <Flex mb="16px" pb="16px">
              <ApproveConfirmButtons
                isApproveDisabled={
                  isProfileCostsLoading || hasMinimumCakeRequired
                }
