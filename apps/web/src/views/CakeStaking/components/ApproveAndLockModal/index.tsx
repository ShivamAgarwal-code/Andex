...
const SeamlessModal: React.FC<
  React.PropsWithChildren<Omit<ModalProps, "title"> & { title?: string }>
...
export const ApproveAndLockModal: React.FC<ApproveAndLockModalProps> = ({
  ...
  const lockInfo = (
    <LockInfo status={status} amount={cakeLockAmount} week={cakeLockWeeks} />
  );
  const scanLink = cakeLockTxHash ? (
    <ScanLink
      small
      icon={<BscScanIcon />}
      href={getBlockExploreLink(cakeLockTxHash, "transaction", chainId)}
    >
      {t("View on %site%", {
        site: t("Explorer"),
      })}
  ...
  return (
    <ModalV2 isOpen={isOpen} onDismiss={onDismiss}>
      <SeamlessModal>
        {status < ApproveAndLockStatus.LOCK_ANDX_PENDING ? (
          <>
            <PendingModalContent
              title={
                status === ApproveAndLockStatus.APPROVING_TOKEN
                  ? t("Approve ANDX spending")
                  : t("Confirm Lock")
              }
              subTitle={
                status === ApproveAndLockStatus.APPROVING_TOKEN
        ...
        {status === ApproveAndLockStatus.UNLOCK_ANDX ? (
          <PendingModalContent title={t("Confirm unlock")} />
        ) : null}

        {status === ApproveAndLockStatus.MIGRATE ? (
          <PendingModalContent title={t("Confirm migrate")} />
        ) : null}

        {[
          ApproveAndLockStatus.UNLOCK_ANDX_PENDING,
          ApproveAndLockStatus.MIGRATE_PENDING,
        ].includes(status) ? (
