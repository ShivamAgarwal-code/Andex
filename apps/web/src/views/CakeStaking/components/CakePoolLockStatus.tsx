...
export const CakePoolLockInfo = () => {
  ...
  const cakeAmount = useMemo(
    () => Number(formatBigInt(lockedAmount)),
    [lockedAmount]
  );
    ...
    <FlexGap flexDirection="column" gap="24px" margin={24}>
      <RowBetween>
        ...
        <Tag
          variant="failure"
          scale="sm"
          startIcon={<WarningIcon color="white" />}
          px="8px"
        >
          {t("Migration Needed")}
        </Tag>
      ...
      <StyledLockedCard gap="16px">
        <RowBetween>
          ...
          <AutoColumn>
            ...
            <Text fontSize={12}>
              {t("on")}{" "}
              {formatTime(Number(dayjs.unix(Number(roundedEndTime || 0))))}
            </Text>
          </AutoColumn>
        </RowBetween>
      ...
      <Message variant="warning" icon={<InfoFilledIcon color="#D67E0A" />}>
        <Text as="p" color="#D67E0A">
          {t(
            "Migrate your ANDX staking position to veANDX and enjoy the benefits of weekly ANDX yield, revenue share, gauges voting, farm yield boosting, participating in IFOs, and so much more!"
          )}
        </Text>
      </Message>
    </FlexGap>
