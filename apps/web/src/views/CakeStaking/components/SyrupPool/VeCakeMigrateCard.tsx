...
export const VeCakeMigrateCard: React.FC<{
  isTableView?: boolean;
  lockEndTime?: string;
}> = memo(({ isTableView, lockEndTime }) => {
      ...
      <Flex alignItems="center" style={{ gap: 10 }}>
        ...
        <Box>
          <Text color="white" bold fontSize={14} pr="20px">
            {isMigratedToVeCake ? (
              t("Your ANDX pool position has been migrated to veANDX.")
            ) : (
              <>
                {t(
                  "All fixed term staking positions must migrate to veANDX to continue receiving rewards."
                )}
                <LearnMoreLink />
              </>
            )}
          ...
          {isMigratedToVeCake && (
            <Text mt="10px" color="white" bold fontSize={14} pr="20px">
              {t(
                "Extending or adding ANDX is not available for migrated positions. You will be able to withdraw ANDX when the lock ends."
              )}
              <LearnMoreLink />
            </Text>
          )}
