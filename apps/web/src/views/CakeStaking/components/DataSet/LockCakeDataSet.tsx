...
export const LockCakeDataSet = () => {
  ...
  const amountInputBN = useMemo(
    () => getDecimalAmount(new BN(cakeLockAmount || 0)),
    [cakeLockAmount]
  );
  const amountLockedBN = useMemo(
    () => getBalanceAmount(new BN(cakeLockedAmount.toString() || "0")),
    [cakeLockedAmount]
  );
  ...
  const veCakeAmount = useMemo(() => {
    return getBalanceAmount(veCakeBalance).plus(
      getVeCakeAmount(cakeLockAmount, cakeUnlockTime - currentTimestamp)
    );
  ...
  return (
    <DataBox gap="8px">
      ...
      <DataRow label={t("ANDX to be locked")} value={amount.toFixed(2)} />
      <DataRow
        label={
          <Tooltips
            content={t(
