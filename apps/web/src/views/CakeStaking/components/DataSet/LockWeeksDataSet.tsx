...
export const LockWeeksDataSet = () => {
  ...
  const { cakeLockExpired, cakeUnlockTime, nativeCakeLockedAmount } =
    useCakeLockStatus();
  const currentTimestamp = useCurrentBlockTimestamp();
  const veCakeAmountBN = useMemo(() => {
    const duration =
      cakeUnlockTime - currentTimestamp + Number(cakeLockWeeks || 0) * WEEK;
  ...
  const factor =
    veCakeAmountBN && veCakeAmountBN.gt(0)
      ? `${veCakeAmountBN
          .div(nativeCakeLockedAmount.toString())
          .toPrecision(2)}x`
      : "0.00x";

  const newUnlockTimestamp = useRoundedUnlockTimestamp(
