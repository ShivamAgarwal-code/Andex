...
import {
  useRevenueSharingCakePool,
  useRevenueSharingVeCake,
} from "../hooks/useRevenueSharingProxy";
...
export const CakeRewardsCard = ({ onDismiss }) => {
  ...
  const yourShare = useMemo(
    () => getBalanceAmount(new BigNumber(balanceOfAt)).toNumber(),
    [balanceOfAt]
  );
  ...
  const showYourSharePercentage = useMemo(
    () => new BigNumber(totalSupplyAt).gt(0),
    [totalSupplyAt]
  );

  const { availableClaim: availableClaimFromCakePool } =
    useRevenueSharingCakePool();
  ...
  const availableRevenueSharingCakeUsdValue = useMemo(
    () =>
      new BigNumber(availableRevenueSharingCake)
        .times(cakePriceBusd)
        .toNumber(),
  ...
  const totalAvailableClaim = useMemo(
    () =>
      getBalanceAmount(
        new BigNumber(availableClaim).plus(availableClaimFromCakePool)
      ).toNumber(),
  ...
  const showExpireSoonWarning = useMemo(() => {
    const endTime = new BigNumber(nextDistributionTimestamp).plus(
      ONE_WEEK_DEFAULT
    );
            ...
            <Box padding={16}>
                  ...
                  <BenefitsTooltipsText
                    ...
                    tooltipComponent={
                      <>
                        <Text>
                          {t(
                            "The virtual shares you currently own and the % against the whole revenue sharing pool."
                          )}
                        </Text>
                        <Text m="4px 0">
                          {t(
                ...
                <Flex mt="8px" flexDirection="row" alignItems="center">
                  <BenefitsTooltipsText
                    ...
                    tooltipComponent={
                      <Text>
                        {t(
                          "Time remaining until the next revenue distribution and share updates."
                        )}
                      </Text>
                    }
                  />
                ...
                <Flex mt="8px" flexDirection="row" alignItems="center">
                  <BenefitsTooltipsText
                    ...
                    tooltipComponent={
                      <Text>
                        {t(
                          "The time of the last revenue distribution and shares update."
                        )}
                      </Text>
                    }
                  />
              ...
              {showNoCakeAmountWarning && (
                <Message
                  ...
                  padding="8px"
                  mt="8px"
                  icon={<WarningIcon color="failure" />}
                >
                  <MessageText lineHeight="120%">
                    {t(
                      "You need to update your staking in order to start earning from protocol revenue sharing."
                    )}
                  </MessageText>
                </Message>
              )}
...
const ClaimButton: React.FC<{
  ...
  const isReady = useMemo(
    () => new BigNumber(availableClaim).gt(0) && !isPending,
    [availableClaim, isPending]
  );
  ...
  const handleClaim = useCallback(async () => {
    try {
      ...
      const receipt = await fetchWithCatchTxError(() =>
        contract.write.claimMultiple([revenueSharingPools, account], {
          account,
