...
export const LockedVeCakeStatus: React.FC<{
  ...
  const proxyCake = useMemo(
    () => getBalanceNumber(proxyBalance),
    [proxyBalance]
  );
    ...
    <Box maxWidth={["100%", "100%", "369px"]} width="100%">
      <Card isActive>
        <CardHeader>
          <RowBetween>
            <AutoColumn>
              <Heading color="text">{t("My VeANDX")}</Heading>
              <Tooltips
                content={
                  proxyBalance.gt(0) ? (
                    <DualStakeTooltip
                      nativeBalance={balanceBN}
                      proxyBalance={proxyCake}
                    />
                  ) : (
                    <SingleStakeTooltip />
                  )
            ...
            <img
              srcSet="/images/cake-staking/token-vecake.png 2x"
              alt="token-vecake"
            />
          </RowBetween>
        </CardHeader>
  ...
  return (
    <FlexGap flexDirection="column" gap="24px" margin={24}>
      ...
      {cakePoolLocked ? (
        <>
          {cakePoolLockExpired ? (
            <Message
              variant="warning"
              icon={<InfoFilledIcon color="warning" />}
            >
              <Text as="p">
                {t(
                  "ANDX Pool migrated position has unlocked. Go to the pool page to withdraw, add ANDX into veANDX to increase your veANDX benefits."
                )}
              </Text>
            </Message>
          ) : (
            <Message
              variant="primary"
              icon={<InfoFilledIcon color="secondary" />}
            >
              <AutoColumn gap="8px">
                <Text as="p">
                  {t(
                    "Position migrated from ANDX Pool can not be extended or topped up. To extend or add more ANDX, set up a native veANDX position."
                  )}
                </Text>
                <LearnMore />
              </AutoColumn>
      ...
      {cakeLockExpired && !cakePoolLockExpired ? (
        <Message
          variant="warning"
          icon={<InfoFilledIcon color={CUSTOM_WARNING_COLOR} />}
        >
          <Text as="p" color={CUSTOM_WARNING_COLOR}>
            {t(
              "Renew your veANDX position to continue enjoying the benefits of weekly ANDX yield, revenue share, gauges voting, farm yield boosting, participating in IFOs, and so much more!"
            )}
          </Text>
        </Message>
      ) : null}
      {!cakeLockExpired ? (
        <Flex justifyContent="center">
          <img
            src="/images/cake-staking/my-cake-bunny.png"
            alt="my-cake-bunny"
            width="254px"
          />
        </Flex>
...
const SingleStakeTooltip = () => {
  ...
  return (
    <>
      {t(
        "veANDX is calculated with number of ANDX locked, and the remaining time against maximum lock time."
      )}
      <LearnMore />
    </>
  );
...
const DualStakeTooltip: React.FC<{
  ...
  return (
    <>
      ...
      <ul>
        <li>
          {t("Native:")} {formatNumber(nativeBalance)} veANDX
        </li>
        <li>
          {t("Migrated:")} {formatNumber(proxyBalance)} veANDX
        </li>
      </ul>
      <br />
      <LearnMore />
...
const ProxyUnlockTooltip: React.FC<{
  ...
  return (
    <>
      {t(
        proxyExpired
          ? "%amount% ANDX from ANDX Pool migrated position is already unlocked. Go to the pool page to withdraw these ANDX."
          : "%amount% ANDX from ANDX Pool migrated position will unlock on %expiredAt%.",
        {
          amount: proxyCake,
...
export const CakeLocked: React.FC<{
  ...
  const nativeCake = useMemo(
    () => Number(formatBigInt(nativeCakeLocked, 18)),
    [nativeCakeLocked]
  );
  ...
  const proxyCake = useMemo(
    () => Number(formatBigInt(proxyCakeLocked, 18)),
    [proxyCakeLocked]
  );
      ...
      <Tooltips
        content={
          <ProxyUnlockTooltip
            ...
            proxyCake={proxyCake}
            proxyUnlockTime={proxyUnlockTime}
          />
        }
      >
        <UnderlinedBalance
  ...
  const [unlocked, unlockTime, unlockTimeToNow] = useMemo(() => {
    ...
    if (!nativeLocked && proxyLocked) {
      return [
        proxyExpired,
        proxyUnlockTime,
        proxyUnlockTime ? dayjs.unix(proxyUnlockTime).from(nowDay, true) : "",
      ];
    }
    return [
      nativeExpired,
      nativeUnlockTime,
      nativeUnlockTime ? dayjs.unix(nativeUnlockTime).from(nowDay, true) : "",
    ];
  }, [
    nativeExpired,
    nativeLocked,
...
const UnderlinedBalance = styled(Balance).withConfig({
