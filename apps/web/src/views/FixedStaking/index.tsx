...
import {
  useStakedPools,
  useStakedPositionsByUser,
} from "./hooks/useStakedPools";
...
const FixedStaking = () => {
  ...
  const displayPoolsIndex = useMemo(
    () => displayPools.map((p) => p.poolIndex),
    [displayPools]
  );
  ...
  const poolGroup = useMemo(() => {
    return Object.keys(groupPoolsByToken).reduce((poolGroupResult, key) => {
      ...
      const minLockDayPercent = min(
        pools.map((p) => p.lockDayPercent || p.boostDayPercent)
      );
      const maxLockDayPercent = max(
        pools.map((p) => p.boostDayPercent || p.lockDayPercent)
      );

      const totalDeposited = pools.reduce(
        (sum, p) => sum.plus(p.totalDeposited),
        new BigNumber(0)
      );
  ...
  const sortedPoolGroup = useMemo(() => {
    const [first, last] = partition(Object.keys(poolGroup), (poolAddress) =>
      ...
      ].includes(poolAddress as Address)
    );
            ...
            {sortedPoolGroup.map((key) => (
              <FixedStakingRow
                ...
                stakedPositions={stakedPositions.filter(
                  ({ pool: stakedPool }) =>
                    stakedPool.token.address === poolGroup[key].token.address
                )}
                pool={poolGroup[key]}
              />
            ))}
