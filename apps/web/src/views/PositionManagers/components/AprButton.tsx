...
export const AprButton = memo(function YieldInfo({
  ...
  const tokenBalance = useMemo(
    () =>
      new BigNumber(
        Number(((userLpAmounts ?? 0n) * 10000n) / (precision ?? 1n)) / 10000 ??
          0
      ).times(tokenBalanceMultiplier),
  ...
  const tokenPrice = useMemo(
    () =>
      totalStakedInUsd /
      (Number(((totalSupplyAmounts ?? 0n) * 10000n) / (precision ?? 1n)) /
        10000 ?? 0),
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <>
      ...
      <Text lineHeight="120%" mt="20px">
        {t(`Calculated based on previous %days% days average data.`, {
          days: 1,
        })}
      </Text>
    </>,
