...
const GridCell = styled(Flex)<{ isTopPosition: boolean }>`
  ...
  ${({ theme, isTopPosition }) =>
    isTopPosition && `background-color: ${theme.colors.warning2D};`}
`;
...
const LeaderboardRow: React.FC<
  React.PropsWithChildren<LeaderboardRowProps>
> = ({ bidder, cakePriceBusd, isMobile, index, auctionId }) => {
            ...
            {!isMobile && (
              <>
                {shouldUseV3Format ? (
                  <>
                    <Text mr="3px">
                      ({v3FarmAuctionConfig?.[index]?.[0]}% fee tier)
                    </Text>
                    <Text>
