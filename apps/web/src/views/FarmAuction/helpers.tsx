...
export const sortAuctionBidders = (
  bidders: readonly BidsPerAuction[],
  auction?: Auction
): Bidder[] => {
  const sortedBidders = orderBy(
    bidders,
    (bidder) => Number(bidder.amount),
    "desc"
  ).map((bidder, index) => {
  ...
  return sortedBidders.map((bidder, index, unadjustedBidders) => {
    ...
    const samePositionAsAbove =
      index === 0
        ? false
        : bidder.amount === unadjustedBidders[index - 1].amount;
    adjustedPosition = samePositionAsAbove
      ? adjustedPosition
      : adjustedPosition + 1;
...
const getAuctionStatus = (
  ...
  if (
    contractStatus === FarmAuctionContractStatus.Pending &&
    !startBlock &&
    !endBlock
  ) {
  ...
  if (
    contractStatus === FarmAuctionContractStatus.Open &&
...
const getDateForBlock = async (currentBlock: number, block: number) => {
  ...
  if (currentBlock > block) {
    try {
      ...
      }).getBlock({ blockNumber: BigInt(block) });
...
export const processAuctionData = async (
  ...
  const processedAuctionData = {
    ...
    initialBidAmount: bigIntToBigNumber(auctionResponse.initialBidAmount)
      .div(DEFAULT_TOKEN_DECIMAL)
      .toNumber(),
    leaderboardThreshold: bigIntToBigNumber(
      auctionResponse.leaderboardThreshold
    ),
  ...
  const currentBlock =
    currentBlockNumber ||
    Number(
      await publicClient({ chainId: ChainId.MODE_MAINNET }).getBlockNumber()
    );
...
export const processBidderAuctions = (
  bidderAuctions: ContractFunctionResult<
    typeof farmAuctionABI,
    "viewBidderAuctions"
  >
