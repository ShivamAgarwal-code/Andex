...
function Remove({ tokenId }: { tokenId?: bigint }) {
  ...
  const { percent } = useLocalSelector<{ percent: number }>((s) => s) as {
    percent: number;
  };
  ...
  const { tokenIds: stakedTokenIds, loading: tokenIdsInMCv3Loading } =
    useV3TokenIdsByAccount(masterchefV3?.address, account);
  ...
  const interfaceManager = isStakedInMCv3
    ? MasterChefV3
  ...
  const onRemove = useCallback(async () => {
    ...
    const { calldata, value } = interfaceManager.removeCallParameters(
      positionSDK,
      {
    ...
    publicClient?.estimateGas(txn).then((gas) => {
      fetchWithCatchTxError(() =>
        ...
        .then((response) => {
          if (response?.status) {
            const amount0 = formatRawAmount(
              liquidityValue0.quotient.toString(),
              liquidityValue0.currency.decimals,
              4
            );
            const amount1 = formatRawAmount(
              liquidityValue1.quotient.toString(),
              liquidityValue1.currency.decimals,
              4
            );
  ...
  const price0 = useStablecoinPrice(
    liquidityValue0?.currency?.wrapped ?? undefined,
    { enabled: !!feeValue0 }
  );
  const price1 = useStablecoinPrice(
    liquidityValue1?.currency?.wrapped ?? undefined,
    { enabled: !!feeValue1 }
  );
  ...
  const [onPresentRemoveLiquidityModal] = useModal(
    <TransactionConfirmationModal
      ...
      pendingText={`Removing ${liquidityValue0?.toSignificant(6)} ${
  ...
  const showCollectAsWNative = Boolean(
    liquidityValue0?.currency &&
      ...
      (liquidityValue0.currency.isNative ||
        ...
        WNATIVE[liquidityValue0.currency.chainId]?.equals(
          liquidityValue0.currency.wrapped
        ) ||
        WNATIVE[liquidityValue1.currency.chainId]?.equals(
        ...
        <CardBody>
            ...
            <Box>
              <Flex>
                ...
                <Heading ml="8px" as="h2">
                  {liquidityValue0?.currency?.symbol}-
                  {liquidityValue1?.currency?.symbol} LP
                </Heading>
              </Flex>
          ...
          <AutoColumn gap="8px" mb="16px">
            ...
            <LightGreyCard>
              ...
              >
                ...
                <Flex>
                  <Text small>
                    {formatCurrencyAmount(liquidityValue0, 4, locale)}
                  </Text>
                </Flex>
              </Flex>
              ...
              >
                ...
                <Flex>
                  <Text small>
                    {formatCurrencyAmount(liquidityValue1, 4, locale)}
                  </Text>
                </Flex>
              </Flex>
              <Flex justifyContent="flex-end" mb="8px">
              ...
              >
                ...
                <Flex>
                  <Text small>
                    {formatCurrencyAmount(feeValue0, 4, locale)}
                  </Text>
                </Flex>
              </Flex>
              <Flex justifyContent="flex-end" mb="8px">
              ...
              >
                ...
                <Flex>
                  <Text small>
                    {formatCurrencyAmount(feeValue1, 4, locale)}
                  </Text>
                </Flex>
              </Flex>
              <Flex justifyContent="flex-end" mb="8px">
          ...
          {isStakedInMCv3 ? (
            <Message variant="primary" mb="20px">
              {t(
                "This liquidity position is currently staking in the Farm. Adding or removing liquidity will also harvest any unclaimed ANDX to your wallet."
              )}
            </Message>
          ) : null}
