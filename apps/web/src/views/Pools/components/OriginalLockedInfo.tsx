...
const OriginalLockedInfo: React.FC<
  React.PropsWithChildren<OriginalLockedInfoProps>
> = ({ pool }) => {
  const {
    t,
  ...
  const originalLockedAmountText =
    originalLockedAmount > 0.01 ? originalLockedAmount.toFixed(2) : "<0.01";
  const originalUsdValueText =
    originalUsdValue > 0.01 ? `~${originalUsdValue.toFixed(2)}` : "<0.01";
