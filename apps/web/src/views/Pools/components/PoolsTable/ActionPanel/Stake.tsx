...
const Staked: React.FunctionComponent<
  React.PropsWithChildren<StackedActionProps>
> = ({ pool }) => {
  const {
    sousId,
    stakingToken,
  ...
  const { handleApprove: handlePoolApprove, pendingTx: pendingPoolTx } =
    useApprovePool(stakingTokenContract, sousId, earningToken.symbol);

  const { isVaultApproved, setLastUpdated } =
    useCheckVaultApprovalStatus(vaultKey);
  const { handleApprove: handleVaultApprove, pendingTx: pendingVaultTx } =
  ...
  const { lockEndDate, remainingTime, burnStartTime } =
    useUserDataInVaultPresenter({
      lockStartTime:
  ...
  const needsApproval = vaultKey
    ? !isVaultApproved
    : !allowance.gt(0) && !isBnbPool;
  ...
  const [onPresentVaultStake] = useModal(
    <VaultStakeModal stakingMax={stakingTokenBalance} pool={pool} />
  );
  ...
  const { notMeetRequired, notMeetThreshold } =
    useProfileRequirement(profileRequirement);
                ...
                <Box position="relative">
                  <Flex>
                    <Balance
                      ...
                      decimals={5}
                      value={
                        vaultKey ? cakeAsNumberBalance : stakedTokenBalance
                      }
                    />
                    {isLocked ? (
                      <>
                  ...
                  <SkeletonV2
                    isDataReady={Number.isFinite(
                      vaultKey
                        ? stakedAutoDollarValue
