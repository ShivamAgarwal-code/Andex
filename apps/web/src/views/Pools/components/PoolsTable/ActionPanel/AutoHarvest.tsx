...
const AutoHarvestAction: React.FunctionComponent<
  React.PropsWithChildren<AutoHarvestActionProps>
> = ({ pool }) => {
  ...
  const { secondDuration, weekDuration } = useUserDataInVaultPresenter({
    lockStartTime:
      vaultKey === VaultKey.CakeVault
        ? (vaultData as DeserializedLockedCakeVault).userData?.lockStartTime ??
          "0"
        : "0",
    lockEndTime:
      vaultKey === VaultKey.CakeVault
  ...
  const [onPresentViewBenefitsModal] = useModal(
    <BenefitsModal
      pool={pool}
      userData={(vaultData as DeserializedLockedCakeVault)?.userData}
    />,
    true,
    false,
    "revenueModal"
      ...
      <Flex justifyContent="space-between">
        <Box width="100%">
          ...
          <ActionContent>
                ...
                {hasAutoEarnings ? (
                  <>
                    <Flex>
                      ...
                      />
                      {tagTooltipVisibleOfRecentProfit &&
                        tagTooltipOfRecentProfit}
                      <HelpIconWrapper ref={tagTargetRefOfRecentProfit}>
                        <HelpIcon ml="4px" color="textSubtle" />
                      </HelpIconWrapper>
                    </Flex>
                    {Number.isFinite(earningTokenPrice) &&
                      earningTokenPrice > 0 && (
                        <BalanceWithLoading
                          display="inline"
                          fontSize="12px"
            ...
            >
              {[VaultPosition.Flexible, VaultPosition.None].includes(
                vaultPosition
              ) && (
      ...
      {vaultKey === VaultKey.CakeVault &&
        ...
        isInitialization && (
          <Button
            ...
            width="100%"
            variant="secondary"
            onClick={onPresentViewBenefitsModal}
          >
            {t("View Benefits")}
          </Button>
        )}
