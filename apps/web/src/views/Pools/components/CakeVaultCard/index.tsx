...
export const CakeVaultDetail: React.FC<
  React.PropsWithChildren<CakeVaultDetailProps>
> = ({
  isLoading = false,
  account,
  pool,
    ...
    <>
      <StyledCardBody isLoading={isLoading}>
        {vaultPosition >= VaultPosition.LockedEnd && (
          <VeCakeUpdateCard isLockEndOrAfterLock />
        )}
          ...
          <>
            ...
            {vaultPosition !== VaultPosition.None && (
              <FlexGap
                ...
                gap="24px"
                flexDirection={
                  accountHasSharesStaked ? "column-reverse" : "column"
                }
              >
                <Box>
                  {account && (
      ...
      <Pool.PoolCardHeader isStaking={accountHasSharesStaked}>
        {!showSkeleton || (totalStaked && totalStaked.gte(0)) ? (
          <>
            ...
            />
            <TokenPairImage
              {...vaultPoolConfig[pool.vaultKey].tokenImage}
              width={64}
              height={64}
            />
          </>
