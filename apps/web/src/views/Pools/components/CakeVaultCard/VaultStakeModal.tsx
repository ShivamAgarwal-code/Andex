...
import {
  ...
  useVaultApprove,
} from "views/Pools/hooks/useApprove";
...
const VaultStakeModal: React.FC<
  React.PropsWithChildren<VaultStakeModalProps>
> = ({
  pool,
  stakingMax,
  performanceFee,
  ...
  const vaultPoolContract = "" as any;
  ...
  const {
    ...
    userData: {
      ...
      balance: { cakeAsBigNumber: new BigNumber(0) },
    },
  };
  ...
  const formattedUsdValueStaked =
    cakePriceBusd.gt(0) && stakeAmount
      ? formatNumber(usdValueStaked.toNumber())
      : "";
  const { flexibleApy } = useVaultApy();
  const { allowance, setLastUpdated } = useCheckVaultApprovalStatus(vaultKey);
  const { handleApprove: handleCakeApprove, pendingTx: cakePendingTx } =
    useVaultApprove(vaultKey, setLastUpdated);
  ...
  const getTokenLink = stakingToken.address
    ...
    : "/swap";
  const convertedStakeAmount = getDecimalAmount(
  ...
  const handleStakeInputChange = (input: string) => {
    if (input) {
      const convertedInput = new BigNumber(input).multipliedBy(
        getFullDecimalMultiplier(stakingToken.decimals)
      );
  ...
  const handleWithdrawal = async () => {
    ...
    const receipt = await fetchWithCatchTxError(() => {
      ...
      if (pool.vaultKey === VaultKey.CakeFlexibleSideVault) {
        const { sharesAsBigNumber } = convertCakeToShares(
        ...
        return callWithGasPrice(
          vaultPoolContract,
          "withdraw",
          [parseInt(sharesAsBigNumber.toString())],
          callOptions
        );
      }
  ...
  const handleDeposit = async (lockDuration = 0) => {
    const receipt = await fetchWithCatchTxError(() => {
      ...
      // as suggested here https://github.com/ChainSafe/web3.js/issues/2077
      const extraArgs =
        pool.vaultKey === VaultKey.CakeVault ? [lockDuration.toString()] : [];
  ...
  return (
    ...
    >
      ...
      <BalanceInput
        ...
        currencyValue={
          cakePriceBusd.gt(0) && `~${formattedUsdValueStaked || 0} USD`
        }
      ...
      {needEnable ? (
        <Button
          ...
          endIcon={
            cakePendingTx ? <AutoRenewIcon spin color="currentColor" /> : null
          }
