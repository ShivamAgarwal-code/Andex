...
const TooltipContainer = styled(Box)`
  ...
  &:before {
    ...
    border-left: ${({ theme }) =>
      theme.isDark ? "10px solid #FFFFFF" : "10px solid #27262c"};
  }
...
const JoinRevenueModal: React.FunctionComponent<
  React.PropsWithChildren<JoinRevenueModalProps>
> = ({ refresh, onDismiss }) => {
  ...
  // const vaultPool = useVaultPoolByKey(
  //   VaultKey.CakeVault
  // ) as DeserializedLockedCakeVault;
  const vaultPool = {} as any;
      ...
      <Box padding="0 13px">
        ...
        <Box>
          <Text as="span" fontSize={12} color="textSubtle">
            {t(
              "Once updated, you need to wait a full distribution cycle to start claiming rewards."
            )}
          </Text>
          <InlineLink
            fontSize={12}
