...
const LockedBenefits = () => {
  ...
  const lockedApy = useMemo(
    () => getLockedApy(secondDuration),
    [getLockedApy, secondDuration]
  );
  const boostFactor = useMemo(
    () => getBoostFactor(secondDuration),
    [getBoostFactor, secondDuration]
  );
  const delApy = useMemo(
    () => new BigNumber(lockedApy).div(boostFactor).toNumber(),
    [lockedApy, boostFactor]
  );
        ...
        <Box padding={16}>
          ...
          <Box mt="8px">
            <Flex mt="8px" flexDirection="row" alignItems="center">
              ...
              <Text ml="2px" as="del" bold>{`${Number(delApy).toFixed(
                2
              )}%`}</Text>
            </Flex>
            <BenefitsText
