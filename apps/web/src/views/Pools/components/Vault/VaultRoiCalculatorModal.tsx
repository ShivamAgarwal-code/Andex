...
export const VaultRoiCalculatorModal = ({
  ...
  initialView?: number;
} & Partial<RoiCalculatorModalProps>) => {
  // const {
  //   userData: {
...
function LockedRoiStrategy({
  ...
  const { compounding, compoundingFrequency, stakingDuration, mode } =
    state.controls;
  ...
  useEffect(() => {
    if (mode === CalculatorMode.ROI_BASED_ON_PRINCIPAL) {
      ...
      const interest =
        (principalInUSDAsNumber / earningTokenPrice) *
        (+getLockedApy(duration) / 100) *
        (duration / 31449600);
      ...
      dispatch({
        type: "setRoi",
        payload: { roiUSD: roiAsUSD, roiTokens, roiPercentage },
      });
    }
  }, [
    principalAsUSD,
  ...
  useEffect(() => {
    if (mode === CalculatorMode.PRINCIPAL_BASED_ON_ROI) {
      const principalUSD =
        roiUSD / (+getLockedApy(duration) / 100) / (duration / 31449600);
