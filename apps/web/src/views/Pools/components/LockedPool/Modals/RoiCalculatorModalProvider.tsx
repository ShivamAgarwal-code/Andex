...
> = ({ children, lockedAmount }) => {
  ...
  if (showRoiCalculator) {
    return (
      <VaultRoiCalculatorModal
        pool={pool}
        linkLabel={t("Get %symbol%", { symbol: "ANDX" })}
        linkHref={
          stakingTokenAddress
            ? `/swap?outputCurrency=${stakingTokenAddress}`
            : "/swap"
        }
  ...
  return (
    <RoiCalculatorModalContext.Provider value={setShowRoiCalculator}>
      {children}
    </RoiCalculatorModalContext.Provider>
  );
};
