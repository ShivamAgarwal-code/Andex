...
const AddAmountModal: React.FC<
  React.PropsWithChildren<AddAmountModalProps>
> = ({
  onDismiss,
  currentBalance,
  currentLockedAmount,
  ...
  const lockedAmountAsBigNumber = useMemo(
    () =>
      !Number.isNaN(new BigNumber(lockedAmount).toNumber())
  ...
  const currentLockedAmountAsBalance = useMemo(
    () => getBalanceAmount(currentLockedAmount),
    [currentLockedAmount]
  );
  ...
  const usdValueNewStaked = useMemo(
    () =>
      getBalanceNumber(
        totalLockedAmountBN.multipliedBy(stakingTokenPrice),
        stakingToken.decimals
      ),
    [totalLockedAmountBN, stakingTokenPrice, stakingToken.decimals]
  );

  const remainingDuration = dayjs(convertTimeToMilliseconds(lockEndTime)).diff(
    dayjs(),
    "seconds"
  );
  const passedDuration = dayjs().diff(
    dayjs(convertTimeToMilliseconds(lockStartTime)),
    "seconds"
  );
  ...
  const atLeastOneWeekNewDuration = Math.max(
    ONE_WEEK_DEFAULT + MIN_DURATION_BUFFER,
    remainingDuration + passedDuration
  );
