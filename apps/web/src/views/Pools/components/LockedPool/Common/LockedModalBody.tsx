...
const LockedModalBody: React.FC<
  React.PropsWithChildren<LockedModalBodyPropsType>
> = ({
  stakingToken,
  stakingTokenPrice,
  onDismiss,
  ...
  const {
    ...
    prepConfirmArg,
    defaultDuration:
      customLockWeekInSeconds || (isRenew && avgLockDurationsInSeconds),
  });
  const [isMaxSelected, setIsMaxSelected] = useState(false);

  const { isValidAmount, isValidDuration, isOverMax }: ModalValidator =
    useMemo(() => {
      return typeof validator === "function"
        ...
        : {
            isValidAmount:
              lockedAmount?.toNumber() > 0 &&
              currentBalance &&
              getBalanceAmount(currentBalance).gte(lockedAmount),
  ...
  const hasEnoughBalanceToExtend = useMemo(
    () => currentBalance?.gte(ENABLE_EXTEND_LOCK_AMOUNT),
    [currentBalance]
  );

  const needsEnable = useMemo(
    () => cakeNeeded && !hasEnoughBalanceToExtend,
    [cakeNeeded, hasEnoughBalanceToExtend]
  );

  const { allowance, setLastUpdated } = useCheckVaultApprovalStatus(
    VaultKey.CakeVault
  );
  const { handleApprove, pendingTx: approvePendingTx } = useVaultApprove(
    VaultKey.CakeVault,
    setLastUpdated
  );
  ...
  const [showEnableConfirmButtons, setShowEnableConfirmButtons] =
    useState(needsEnable);
