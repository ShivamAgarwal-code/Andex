...
const Overview: React.FC<React.PropsWithChildren<OverviewPropsType>> = ({
  ...
  const lockedApy = useMemo(
    () => getLockedApy(duration),
    [getLockedApy, duration]
  );
  const boostFactor = useMemo(
    () => getBoostFactor(duration),
    [getBoostFactor, duration]
  );
  const newLockedApy = useMemo(
    () => (newDuration && getLockedApy(newDuration)) || 0,
    [getLockedApy, newDuration]
  );
  const newBoost = useMemo(
    () => (newDuration && getBoostFactor(newDuration)) || 0,
    [getBoostFactor, newDuration]
  );
  ...
  const newFormattediCake = useMemo(() => {
    ...
    return (
      formatICake({ lockedAmount: amount, duration: lockDuration, ceiling }) ||
      0
    );
        ...
        <LightGreyCard>
          ...
          <TextRow
            ...
            newValue={
              isValidDuration &&
              newDuration &&
              formatSecondsToWeeks(newDuration)
            }
          />
          <BalanceRow
            title={t("Yield boost")}
      ...
      {showLockWarning && (
        <Box mt="16px" maxWidth="370px">
          <Message variant="warning">
            <MessageText>
              {t(
                "You will be able to withdraw the staked ANDX and profit only when the staking position is unlocked"
              )}
            </MessageText>
          </Message>
        </Box>
