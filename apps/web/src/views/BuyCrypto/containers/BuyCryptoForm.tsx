...
export function BuyCryptoForm({
  ...
  const { onFieldAInput, onCurrencySelection, onLimitAmountUpdate } =
    useBuyCryptoActionHandlers();
  ...
  const fetchMinBuyAmounts = useCallback(async () => {
    ...
    const limitAmounts = await fetchMinimumBuyAmount(
      outputCurrencyId,
      inputCurrencyId,
      chainId
    );
    ...
    onFieldAInput(
      toString(
        calculateDefaultAmount(
          ...
          limitAmounts.baseCurrency.code.toUpperCase()
        )
      )
    );
  ...
  return (
    <Box p="4px">
      ...
      <FormContainer>
        ...
        {[ChainId.MODE_MAINNET].includes(chainId) ? (
        ...
        <Text color="textSubtle" fontSize="14px" px="4px">
          {t(
            "Proceed to get live aggregated quotes from a variety of different fiat onramp providers."
          )}
        </Text>
        <GetQuotesButton
