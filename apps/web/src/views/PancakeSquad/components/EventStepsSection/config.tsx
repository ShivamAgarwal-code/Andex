...
const stepsConfigBuilder = ({
  ...
  const { maxPerAddress, maxPerTransaction, maxSupply, pricePerTicket } =
    eventInfos || {};
  const {
    canClaimForGen0,
    ticketsOfUser,
  ...
  const hasProfileActivated = [
    UserStatusEnum.PROFILE_ACTIVE,
    UserStatusEnum.PROFILE_ACTIVE_GEN0,
  ].includes(userStatus);
  ...
  return [
    {
      ...
      bodyText: [
        t(
          "You’ll need an active Andex Profile to buy Minting Tickets and mint a Pancake Squad NFT!"
        ),
      ],
      buttons: (
        <>
    ...
    },
    {
      ...
      bodyText: [
        t(
          "Each NFT costs ANDX to mint. Remember you also need BNB to cover transaction fees too!"
        ),
      ],
      buttons: (
        <>
          <Button
            as="a"
            href="https://andex.vercel.app/swap?outputCurrency=0x0e09fabb73bd3ade0a17ecc321fd13a19e81ce82&chainId=56"
            target="_blank"
            width="100%"
          >
            {t("Buy ANDX")}
    ...
    },
    {
      ...
      buttons: (
        <>
          ...
          {isBuyPhaseFinished && hasData && (
            <ReadyText mt="16px" text={t("Phase Complete!")} />
          )}
        </>
      ),
    },
    {
      ...
      bodyText: [
        t(
          "During this phase, any wallet holding a Squad Ticket can redeem their ticket to claim a Pancake Squad NFT."
        ),
      ],
      buttons: (
        <>
    ...
    },
    {
      ...
      bodyText: [
        t(
          "It’ll take a few days before your bunny’s image is revealed. Just hold tight and wait for the final image!"
        ),
        t(
          "Trading will open before the images are live, but you’ll still be able to check the bunnies’ stats."
        ),
      ],
      buttons: (
        <>
