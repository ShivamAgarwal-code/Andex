...
const BuyTicketsButtons: React.FC<React.PropsWithChildren<BuyTicketsProps>> = ({
  ...
  const {
    ...
    onConfirm: ({ ticketsNumber }) => {
      ...
      return callWithGasPrice(
        nftSaleContract,
        isPreSale ? "buyTicketsInPreSaleForGen0" : "buyTickets",
        [ticketsNumber]
      );
    },
  ...
  const [onPresentEnableModal, onDismissEnableModal] = useModal(
    <ConfirmModal
      ...
      loadingText={t("Please enable ANDX spending in your wallet")}
  ...
  useEffect(
    () => txHashEnablingResult && onPresentEnableModal(),
    [txHashEnablingResult]
  );
  useEffect(
    () => txHashBuyingResult && onPresentConfirmModal(),
    [txHashBuyingResult]
  );
  useEffect(
    () => hasApproveFailed && onDismissEnableModal(),
    [hasApproveFailed]
  );
  useEffect(
    () => hasConfirmFailed && onDismissBuyTicketsModal(),
    [hasConfirmFailed]
  );
  useEffect(
    () => isApproved && setIsUserEnabled && setIsUserEnabled(isApproved),
  ...
  return (
    <>
      ...
      {buyButton === BuyButtonsEnum.READY && (
        <ReadyText
          text={
            isGen0User ? t("Ready for Pre-Sale!") : t("Ready for Public Sale!")
          }
        />
      )}
