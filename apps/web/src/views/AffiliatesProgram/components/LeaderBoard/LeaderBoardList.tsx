...
const LeaderBoardList: React.FC<
  React.PropsWithChildren<LeaderBoardListProps>
> = ({ list, isFetching }) => {
              ...
              <>
                {otherData.map((data, index) => (
                  <tr key={data.address}>
                    ...
                    <Td>
                      <Text bold>{`$${formatNumber(
                        Number(data.metric.totalTradeVolumeUSD),
                        0
                      )}`}</Text>
                    </Td>
                    {/* <Td>
                      <Flex flexDirection="column">
                        <Text bold>{`~ ${formatNumber(Number(data.cakeBalance), 0)} ANDX`}</Text>
