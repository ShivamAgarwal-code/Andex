...
const HistoricalReward: React.FC<
  React.PropsWithChildren<HistoricalRewardProps>
> = ({ isAffiliate }) => {
  ...
  const handleClickClaim = async (
    ...
    try {
      ...
      if (currentTime > new Date(reward.expiryTime).getTime()) {
        const url = isAffiliateClaim
          ? "affiliate-regenerate-signature"
          : "user-regenerate-signature";
        ...
        if (result.status === "success") {
          await callContract(
            isAffiliateClaim,
            result?.claimRequest as ClaimDetail
          );
        } else {
  ...
  const callContract = async (
    ...
    try {
      ...
      const expiryTime = Math.floor(
        new Date(reward.expiryTime).getTime() / 1000
      );
  ...
  return (
    <Flex flexDirection="column" width="100%">
      {chainId !== ChainId.MODE_MAINNET && <WrongNetworkWarning />}
      <>
        {isAffiliate && (
