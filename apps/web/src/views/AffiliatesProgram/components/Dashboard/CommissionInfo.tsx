...
const CardInner = styled(Flex)`
  ...
  ${StyledFlex} {
    &:first-child {
      height: 66px;
      border-right: ${({ theme }) =>
        `solid 1px ${theme.colors.inputSecondary}`};
    }
  }
`;
...
const CommissionInfo: React.FC<
  React.PropsWithChildren<CommissionInfoProps>
> = ({ affiliate }) => {
  ...
  const totalCakeEarned = useMemo(() => {
    const cakeBalance = new BigNumber(totalEarnFeeUSD)
      .div(cakePriceBusd)
      .toNumber();
        ...
        <Box padding={["24px"]}>
          <CardInner mb="28px">
            ...
            <StyledFlex pl="10%">
              ...
              <Text color="textSubtle" fontSize="14px">{`$ ${formatNumber(
                Number(totalEarnFeeUSD)
              )}`}</Text>
            </StyledFlex>
          </CardInner>
          <Box mb="24px">
            ...
            {Number(totalEarnFeeUSD) > 0 && (
              <PieChartContainer chartData={chartData} />
            )}
          </Box>
