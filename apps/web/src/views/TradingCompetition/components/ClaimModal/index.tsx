...
const ClaimModal: React.FC<React.PropsWithChildren<CompetitionProps>> = ({
  ...
  const handleClaimClick = async () => {
    ...
    if (receipt?.status) {
      toastSuccess(
        t("You have claimed your rewards!"),
    ...
    <Modal title={t("Collect Winnings")} onDismiss={onDismiss}>
      ...
      >
        ...
        {canClaimNFT ? (
          ...
          >
            <ImageWrapper>
              <Image
                src={MoDAllBunnies}
                alt="achievement-claim-pcs"
                width={128}
                height={95}
              />
            </ImageWrapper>
            <Text mt="8px">{t("Andex NFT")}</Text>
            <Text
              color="textSubtle"
              mt="8px"
              fontSize="12px"
        ...
        <Text mt="24px" fontSize="12px" color="textSubtle" textAlign="center">
          {t(
            "All prizes will be sent directly to your wallet and user account."
          )}
        </Text>
      </Flex>
    </Modal>
