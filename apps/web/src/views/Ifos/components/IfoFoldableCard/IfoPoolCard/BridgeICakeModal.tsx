...
import {
  ...
  BRIDGE_STATE,
  useBridgeMessageUrl,
  useBridgeSuccessTxUrl,
} from "../../../hooks/useBridgeICake";
import { ICakeLogo, IfoIcon } from "../../Icons";

type Props = {
  // iANDX on source chain to bridge
...
export function BridgeICakeModal({
  icake,
  sourceChainId,
  ifoChainId,
  state,
  ...
  const renderModal = () => {
    switch (state.state) {
      case BRIDGE_STATE.INITIAL:
        return (
          <StyledModal title={t("Bridge iANDX")}>
            <ModalBody>
              <BodyText>
                {t(
...
export function BridgeStateModal({
  state,
  icake,
  sourceChainId,
  ifoChainId,
}: BridgeStateModalProps) {
  const { t } = useTranslation();
  const sourceChainName = useChainName(sourceChainId);
  ...
  const renderTips = () => {
    switch (state.state) {
      ...
      case BRIDGE_STATE.PENDING_SOURCE_CHAIN_TX:
        return (
          <BodyText>{t("Waiting for transaction to be confirmed")}</BodyText>
        );
      case BRIDGE_STATE.PENDING_CROSS_CHAIN_TX:
        return (
          <>
  ...
  return (
    <StyledStateModal title={t("Bridge iANDX")}>
      <ModalBody>
        <Flex
          flexDirection="column"
          justifyContent="flex-start"
        ...
        >
          ...
          {!isSuccess && (
            <StateTitle mt="1rem">
              {t("Bridging %amount% iANDX", {
                amount: formatAmount(icake) || "",
              })}
            </StateTitle>
          )}
          {crossChainInfo}
          <BridgeStateIconDisplay
            mt="0.75rem"
          ...
          {link}
          {isSuccess && (
            <StateTitle mt="1rem">{t("Transaction receipt")}:</StateTitle>
          )}
          {txLink}
        </Flex>
      </ModalBody>
...
export function BridgeStateIconDisplay({
  state,
  srcChainId,
  ifoChainId,
  ...props
}: BridgeStateIconDisplayProps) {
  const content =
    state.state === BRIDGE_STATE.FINISHED ? (
      <>
