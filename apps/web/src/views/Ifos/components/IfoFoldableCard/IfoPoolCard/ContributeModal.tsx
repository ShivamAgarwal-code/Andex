...
const ContributeModal: React.FC<React.PropsWithChildren<Props>> = ({
  ...
  const multiplier = useMemo(
    () => getFullDecimalMultiplier(currency.decimals),
    [currency]
  );
  const valueWithTokenDecimals = new BigNumber(value).times(multiplier);
  const cake = ANDX[ifo.chainId];
  const label = cake ? t("Max. ANDX entry") : t("Max. token entry");
  ...
  const {
    ...
    minAmount: value
      ? parseUnits(value as `${number}`, currency.decimals)
      : undefined,
  ...
  const maximumTokenEntry = useMemo(() => {
    if (!creditLeft || (ifo.version >= 3.1 && poolId === PoolIds.poolBasic)) {
      return limitPerUserInLP?.minus(
        amountTokenCommittedInLP || new BigNumber(0)
      );
    }
    if (limitPerUserInLP?.isGreaterThan(0)) {
      return limitPerUserInLP
        .minus(amountTokenCommittedInLP || new BigNumber(0))
        .isLessThanOrEqualTo(creditLeft)
  ...
  const maximumTokenCommittable = useMemo(() => {
    return maximumTokenEntry?.isLessThanOrEqualTo(userCurrencyBalance)
      ? maximumTokenEntry
      : userCurrencyBalance;
      ...
      <ModalBody maxWidth="360px">
        <Box p="2px">
          <Flex justifyContent="space-between" mb="16px">
            ...
            <Text>{`${formatNumber(
              getBalanceAmount(
                maximumTokenEntry || new BigNumber(0),
                currency.decimals
              ).toNumber(),
              3,
              3
          ...
          <Flex justifyContent="space-between" mb="8px">
            ...
            <Flex flexGrow={1} justifyContent="flex-end">
              <Image
                src={
                  ifo.currency.symbol === "ANDX"
                    ? "/images/cake.svg"
                    : `/images/farms/${currency.symbol
                        .split(" ")[0]
                        .toLowerCase()}.svg`
                }
                width={24}
            ...
            {multiplierValues.map((multiplierValue, index) => (
              <Button
                ...
                onClick={() =>
                  setValue(
                    getBalanceAmount(
                      maximumTokenCommittable.times(multiplierValue)
                    ).toString()
                  )
                }
          ...
          <Text color="textSubtle" fontSize="12px" mb="24px">
            {t(
              "If you don’t commit enough ANDX, you may not receive a meaningful amount of IFO tokens, or you may not receive any IFO tokens at all."
            )}
            <Link
              fontSize="12px"
              display="inline"
          ...
          <ApproveConfirmButtons
            isApproveDisabled={
              isConfirmed || isConfirming || isApproved || !value
            }
