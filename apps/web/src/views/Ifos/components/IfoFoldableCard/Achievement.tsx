...
const SmartContractIcon: React.FC<React.PropsWithChildren<SvgProps>> = (
  props
) => {
  return (
    <Svg
...
const IfoAchievement: React.FC<React.PropsWithChildren<Props>> = ({
  ifo,
  publicIfoData,
}) => {
  ...
  const minLpForAchievement = publicIfoData.thresholdPoints
    ...
    : FIXED_MIN_DOLLAR_FOR_ACHIEVEMENT.div(publicIfoData.currencyPriceInUSD)
        .toNumber()
        .toFixed(3);
      ...
      >
        ...
        <Flex flexDirection="column" ml="8px">
          ...
          {publicIfoData.currencyPriceInUSD.gt(0) ? (
            <Text color="textSubtle" fontSize="12px">
              {t("Commit ~%amount% %symbol% in total to earn!", {
                ...
                symbol:
                  ifo.currency === goerliTestnetTokens.cake ? "ANDX" : "LP",
              })}
            </Text>
          ) : (
          ...
          <FlexGap gap="16px" pt="24px" pl="4px">
            ...
            <Link
              external
              href={getBlockExploreLink(ifo.address, "address", ifo.chainId)}
            >
