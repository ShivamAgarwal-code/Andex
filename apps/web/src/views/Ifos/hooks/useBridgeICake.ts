...
export type BaseBridgeState = {
  state:
    | BRIDGE_STATE.INITIAL
    | BRIDGE_STATE.PENDING_WALLET_SIGN
    | BRIDGE_STATE.PENDING_SOURCE_CHAIN_TX;
};
...
export function useBridgeICake({
  ...
  const infoSender = useContract(INFO_SENDER, pancakeInfoSenderABI, {
    chainId: srcChainId,
  });
  const { receipt, saveTransactionHash, clearTransactionHash, txHash } =
    useLatestBridgeTx(ifoId, srcChainId);
  const message = useCrossChainMessage({
    txHash: receipt?.transactionHash,
    srcChainId,
  });
  const { fetchWithCatchTxError } = useCatchTxError({
    throwUserRejectError: true,
  });
  const isICakeSynced = useMemo(
    () =>
      icake &&
      dstIcake &&
      icake.quotient === dstIcake.quotient &&
      icake.quotient > 0n,
    [icake, dstIcake]
  );
    ...
    try {
      await fetchWithCatchTxError(async () => {
        ...
        const txReceipt = await callWithGasPrice(
          infoSender,
          "sendSyncMsg",
          [getLayerZeroChainId(ifoChainId)],
          {
            value: gasEstimate.quotient,
          }
        );
        saveTransactionHash(txReceipt.hash);
        const summary = `Bridge ${icake?.toExact()} iANDX from ${sourceChainName} to ${ifoChainName}`;
        addTransaction(txReceipt, {
          summary,
          translatableSummary: {
            text: "Bridge %icakeAmount% iANDX from %srcChain% to %ifoChain%",
            data: {
  ...
  const isBridging = useMemo(
    () =>
      state.state !== BRIDGE_STATE.INITIAL &&
...
export function useBridgeMessageUrl(state: BridgeState) {
  return useMemo(
    () =>
      state.state === BRIDGE_STATE.PENDING_CROSS_CHAIN_TX ||
      state.state === BRIDGE_STATE.FINISHED
...
const getLastBridgeTxStorageKey = (
  ifoId: string,
  chainId?: ChainId,
  account?: Address
) =>
  chainId &&
  account &&
  `bridge-icake-tx-hash-latest-${account}-${chainId}-${ifoId}`;

export function useLatestBridgeTx(ifoId: string, chainId?: ChainId) {
  const { address: account } = useAccount();
  const [tx, setTx] = useState<Hash | null>(null);
  const storageKey = useMemo(
    () => getLastBridgeTxStorageKey(ifoId, chainId, account),
    [ifoId, chainId, account]
  );
...
export function useCrossChainMessage({
  txHash,
  srcChainId,
}: CrossChainMeesageParams) {
