...
const CurrentIfo: React.FC<React.PropsWithChildren<TypeProps>> = ({
  activeIfo,
}) => {
  // useFetchIfo()
  const publicIfoData = useGetPublicIfoV7Data(activeIfo);
  const walletIfoData = useGetWalletIfoV7Data(activeIfo);
  const { hasBridged, sourceChainCredit, srcChainId, destChainCredit } =
    useICakeBridgeStatus({
      ifoChainId: activeIfo.chainId,
      ifoCredit: walletIfoData.ifoCredit?.credit,
    });
  const isCrossChainIfo = useMemo(
    () => isCrossChainIfoSupportedOnly(activeIfo.chainId),
    [activeIfo.chainId]
  );
  ...
  const isCommitted = useMemo(
    ...
    [
      poolBasic?.amountTokenCommittedInLP,
