...
> = ({ onDismiss }) => {
  ...
  const [maxPossibleTicketPurchase, setMaxPossibleTicketPurchase] =
    useState(BIG_ZERO);
  ...
  const memoisedUserCake = useMemo(
    () => new BigNumber(stringifiedUserCake),
    [stringifiedUserCake]
  );
  ...
  useEffect(() => {
    ...
    setTicketCostBeforeDiscount(
      costBeforeDiscount.gt(0) ? getFullDisplayBalance(costBeforeDiscount) : "0"
    );
    setTotalCost(
      costAfterDiscount.gt(0) ? getFullDisplayBalance(costAfterDiscount) : "0"
    );
    setDiscountValue(
      discountBeingApplied.gt(0)
        ? getFullDisplayBalance(discountBeingApplied, 18, 5)
        : "0"
    );
  }, [
    ticketsToBuy,
  ...
  const [updateTicket, randomize, tickets, allComplete, getTicketsForPurchase] =
    useTicketsReducer(parseInt(ticketsToBuy, 10), userCurrentTickets);
    ...
    >
      ...
      {account && !hasFetchedBalance ? (
        ...
        <ShortcutButtonsWrapper
          isVisible={
            account && hasFetchedBalance && oneHundredPercentOfBalance >= 1
          }
        >
          ...
          {twentyFivePercentOfBalance >= 1 && (
            <NumTicketsToBuyButton
              onClick={() =>
                handleNumberButtonClick(twentyFivePercentOfBalance)
              }
            >
