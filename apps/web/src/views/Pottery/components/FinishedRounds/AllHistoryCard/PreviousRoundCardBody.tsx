...
const PreviousRoundCardBody: React.FC<
  React.PropsWithChildren<PreviousRoundCardBodyProps>
> = ({ latestRoundId, finishedRoundInfo }) => {
  const {
    t,
  ...
  const isLatest = useMemo(
    () => new BigNumber(latestRoundId).minus(1).eq(roundId),
          ...
          <WinnersContainer>
            {winners &&
              winners.map((address, index) => (
                <Winner key={`${address}-${index}`} address={address} />
              ))}
          </WinnersContainer>
        </Flex>
        <Box width="100%">
