...
const DepositButton: React.FC<React.PropsWithChildren<DepositButtonProps>> = ({
  ...
  const { isPending, handleDeposit } = useDepositPottery(
    depositAmount,
    potteryVaultAddress
  );
  const depositAmountAsBN = new BigNumber(depositAmount).multipliedBy(
  ...
  return (
    <Button
      ...
      isLoading={isPending}
      disabled={
        status !== PotteryDepositStatus.BEFORE_LOCK ||
        depositAmountAsBN.lte(0) ||
        depositAmountAsBN.isNaN()
      }
