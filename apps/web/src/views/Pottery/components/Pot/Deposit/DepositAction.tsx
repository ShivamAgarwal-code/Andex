...
const Container = styled.div<InputProps>`
  ...
  box-shadow: ${({ theme, isWarning }) =>
    isWarning ? theme.shadows.warning : theme.shadows.inset};
`;
...
const DepositAction: React.FC<React.PropsWithChildren<DepositActionProps>> = ({
  ...
  const remainingCakeCanStake = new BigNumber(maxTotalDepositToNumber)
    .minus(totalValueLockedValue)
    .toString();

  const { balance: userCake } = useTokenBalance(ANDX[chainId]?.address);
  const userCakeDisplayBalance = getFullDisplayBalance(userCake, 18, 3);
  const { userNotEnoughCake, notEnoughErrorMessage } =
    useUserEnoughCakeValidator(depositAmount, userCake);

  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    t(
      "ANDX deposit will be diverted to the fixed-term staking pool. Please note that ANDX deposited can ONLY be withdrawn after 10 weeks."
    ),
    {
      placement: "bottom",
    }
  ...
  const onClickMax = () => {
    const userCakeBalance = userCake
  ...
  const isLessThanOneCake = useMemo(
    () => new BigNumber(depositAmount).lt(1),
    [depositAmount]
  );

  const isReachMaxAmount = useMemo(() => {
    return (
      new BigNumber(totalValueLockedValue).eq(maxTotalDepositToNumber) ||
      new BigNumber(remainingCakeCanStake).lt(1)
    );
      ...
      <InputPanel>
        ...
        {isLessThanOneCake && (
          <Text color="failure" fontSize="14px" textAlign="right">
            {t("Please deposit at least 1 ANDX to participate in the Pottery")}
          </Text>
        )}
      </InputPanel>
      <Flex>
        <Flex ml="auto">
          <Text fontSize="12px" color="textSubtle">
            {t("Deposited ANDX will be locked for 10 weeks")}
          </Text>
          <Flex ref={targetRef}>
