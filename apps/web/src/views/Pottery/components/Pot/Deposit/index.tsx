...
const Deposit: React.FC<React.PropsWithChildren> = () => {
  ...
  const currentDeposit = userData.withdrawAbleData.find(
    (data) =>
      data.potteryVaultAddress.toLowerCase() === lastVaultAddress.toLowerCase()
  );
    ...
    <Box>
      <Container>
        ...
        <Flex justifyContent="space-between">
          ...
          <Balance bold decimals={2} value={totalValueLocked} unit=" ANDX" />
        </Flex>
        <Flex justifyContent="space-between">
          ...
          <Balance
            bold
            decimals={2}
            value={getBalanceNumber(maxTotalDeposit)}
            unit=" ANDX"
          />
        </Flex>
      </Container>
