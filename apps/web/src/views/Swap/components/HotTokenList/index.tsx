...
const StyledFlex = styled(Flex)`
  ...
  > div:first-child > div {
    background: ${({ theme }) =>
      theme.isDark ? `rgba(55, 47, 71,0.5)` : `rgba(238, 234, 244, 0.5)`};
    overflow: hidden;
  }
`;
...
const Wrapper = styled.div`
  ...
  ${({ theme }) => theme.mediaQueries.lg} {
    ...
    background: ${({ theme }) =>
      theme.isDark ? "rgba(39, 38, 44, 0.5)" : "rgba(255, 255, 255, 0.5)"};
    border-top: none;
...
const HotTokenList: React.FC<{
      ...
      v2Tokens
        .filter(
          (t) =>
            ...
            t.priceUSDChange !== 0 &&
            t.volumeUSD !== 0 &&
            t.tvlUSD >= LIQUIDITY_FILTER[chainId]
        )
        .map((i) => {
  ...
  return (
    <StyledFlex flexDirection="column">
      <TabToggleGroup>
        <StyledTabToggle
          isActive={dataSource === DataSourceType.V3}
          onClick={() => setDataSource(DataSourceType.V3)}
        >
          {t("V3")}
        </StyledTabToggle>
        <StyledTabToggle
          isActive={dataSource === DataSourceType.V2}
          onClick={() => setDataSource(DataSourceType.V2)}
        >
          {t("V2")}
        </StyledTabToggle>
      </TabToggleGroup>
          ...
          <TokenTable
            ...
            tokenDatas={
              dataSource === DataSourceType.V3
                ? filterFormattedV3Tokens
                : formattedTokens
            }
            type={
              chainId === ChainId.MODE_MAINNET ? "priceChange" : "liquidity"
            }
            defaultSortField={
              chainId === ChainId.MODE_MAINNET
                ? "priceUSDChange"
