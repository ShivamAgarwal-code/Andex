...
const TradingRewardIcon = ({ pairs }: TradingRewardIconProps) => {
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <Flex flexDirection="column" width={300}>
      <Text bold>
        {t(
          "This token contains trading pair(s) eligible for earning ANDX by trading:"
        )}
      </Text>
      <Flex flexDirection="column">
        {pairs.map((pair) => (
          <Text key={pair.lpAddress} bold as="li">{`${pair.lpSymbol} - ${
            pair.feeAmount / 10000
          }%`}</Text>
        ))}
      </Flex>
      <Text mt="10px">
  ...
  return (
    <Box width={24} height={24} mr="4px">
      <Image
        ...
        height={24}
        ref={targetRef}
        alt="token-reward-icon"
        src="/images/swap/token-reward-icon.png"
      />
