...
export const useTokenHighLightList = () => {
  ...
  const allTokensFromWhiteList = useTokenDatasQuery(
    chainWhiteList || [],
    false
  );
  ...
  const allV3TokensFromV3 = useTokensData(
    chainWhiteList || tokenAddresses,
    chainId
  );

  const tokens = useMemo(() => {
    return {
      v2Tokens: (allTokensFromWhiteList || allTokensFromChain)?.map(
        parseV2TokenData
      ),
