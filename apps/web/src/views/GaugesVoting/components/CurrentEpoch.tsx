    ...
    <Card isActive innerCardProps={{ padding: isDesktop ? "1.5em" : "1em" }}>
      <FlexGap gap="8px" flexDirection="column">
        <AutoRow justifyContent="space-between">
          ...
          >
            ...
            >
              <Tooltips
                content={t(
                  "Results for the current epoch will be snapshotted and tallied at 00:00 UTC on %date%.",
                  {
              ...
              <FlexGap gap="2px" alignItems="baseline">
                <Text bold fontSize={16}>
                  {dayjs
                    .unix(nextEpochStart)
                    .from(dayjs.unix(currentTimestamp), true)}
                </Text>
                <Text fontSize={14}>
                  ({dayjs.unix(nextEpochStart).format("DD MMM YYYY")}){" "}
                </Text>
              </FlexGap>
            </FlexGap>
          </FlexGap>
          ...
          >
            ...
            <FlexGap alignItems="baseline" gap="2px">
              ...
              {onTally ? (
                ...
                <Text fontSize={14}>
                  ({dayjs.unix(epochEnd).format("DD MMM YYYY")}){" "}
                </Text>
              )}
            </FlexGap>
          </FlexGap>
        ...
        <AutoRow justifyContent="space-between">
          <Tooltips
            content={t(
              "The total amount of ANDX rewards to distribute to all the gauges according to the final vote results."
            )}
          >
            <TooltipText fontSize={14} color="textSubtle">
              {t("ANDX rewards")}
          ...
          <FlexGap alignItems="baseline" gap="2px">
            ...
            <Text fontSize={14}>
              (
              {getFullDisplayBalance(
                new BN(weeklyRewards).div(2 * 7 * 24 * 60 * 60),
                18,
                3
              )}{" "}
              ANDX/sec){" "}
        ...
        <AutoRow justifyContent="space-between">
          <Tooltips
            content={t("The total number of veANDX votes being casted.")}
          >
            <TooltipText fontSize={14} color="textSubtle">
              {t("Total votes")}
            </TooltipText>
          ...
          <Balance
            bold
            fontSize={16}
            value={getBalanceNumber(new BN(totalWeight.toString()))}
            unit=" veANDX"
          />
        </AutoRow>
      </FlexGap>
