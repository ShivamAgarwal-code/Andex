...
const Scrollable = styled.div.withConfig({
...
export const VoteTable = () => {
  ...
  const voteSum = useMemo(() => {
    return Object.values(votes).reduce(
      (acc, cur) => acc + Number(cur?.power),
      0
    );
  }, [votes]);
  const lockedPowerPercent = useMemo(() => {
    return Object.values(votes).reduce(
      (acc, cur) => acc + (cur?.locked ? Number(cur?.power) : 0),
      0
    );
  }, [votes]);
  ...
  const onVoteChange = (value: UserVote, isMax?: boolean) => {
    ...
    if (isMax) {
      ...
      newVotes[hash].power =
        100 - sum > 0 ? String(Number((100 - sum).toFixed(2))) : "0";
    } else {
      newVotes[hash] = value;
    }
    setVotes(newVotes);
  ...
  const disabled = useMemo(() => {
    const lockedSum = Object.values(votes).reduce(
      (acc, cur) => acc + (cur?.locked ? Number(cur?.power) : 0),
      0
    );
    const newAddSum = Object.values(votes).reduce(
      (acc, cur) => acc + (!cur?.locked ? Number(cur?.power) : 0),
      0
    );
  ...
  return (
    <>
      ...
      <Card
        innerCardProps={{
          padding: isDesktop ? "2em" : "0",
          paddingTop: isDesktop ? "1em" : "0",
        }}
        mt="2em"
      >
        {gauges}
        ...
        {showNoCakeLockedWarning ? (
          ...
          >
            <Message variant="warning" showIcon>
              <AutoColumn gap="8px">
                <Text>{t("You have no locked ANDX.")}</Text>
                <FlexGap alignItems="center" gap="0.2em">
                  {t("To cast your vote, ")}
                  <Link href="/cake-staking">
                    <Text bold>{t("lock your ANDX")}</Text>
                  </Link>
          ...
          >
            <Message variant="warning" showIcon>
              <AutoColumn gap="8px">
                <Text>
                  {t(
                    "Votes are currently being adjusted and tallied. No more votes can be casted."
                  )}
                </Text>
              </AutoColumn>
            </Message>
