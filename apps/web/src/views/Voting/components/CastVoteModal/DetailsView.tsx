...
const FixedTermWrapper = styled(Box)<{ expired?: boolean }>`
  ...
  background: ${({ theme, expired }) =>
    expired
      ...
      : "linear-gradient(180deg, #53dee9, #7645d9)"};
...
const FixedTermCardInner = styled(Box)<{ expired?: boolean }>`
  ...
  &:before {
    ...
    border-radius: ${({ theme }) => theme.radii.default};
    background: ${({ theme, expired }) =>
      expired ? "rgba(255, 178, 55, 0.098)" : theme.colors.gradientBubblegum};
  }
`;
...
const DetailsView: React.FC<React.PropsWithChildren<DetailsViewProps>> = ({
  ...
  const isBoostingExpired = useMemo(() => {
    return (
      lockedEndTime !== 0 &&
      new BigNumber(blockTimestamp?.toString()).gte(lockedEndTime)
    );
      ...
      {Number.isFinite(lockedCakeBalance) && (
        <Box>
          <Text>
            {isBoostingExpired
              ? t(
                  "Your vANDX boosting was expired at the snapshot block. Renew your fixed-term staking position to activate the boost for future voting proposals."
                )
              : t(
                  "Voting power is calculated using the staking amount and remaining staking duration of the fixed-term ANDX staking position at the block."
                )}
          </Text>
          <Text bold m="10px 0">
            {`${t("ANDX locked:")} ${formatNumber(lockedCakeBalance, 0, 2)}`}
          </Text>
  ...
  return (
    <ModalInner mb="0">
      <Text as="p" mb="24px" fontSize="14px" color="textSubtle">
        {t(
          "Your voting power is determined by the amount of ANDX you held and the remaining duration on the fixed-term staking position (if you have one) at the block detailed below. ANDX held in other places does not contribute to your voting power."
        )}
      </Text>
      <Text
        color="secondary"
      ...
      >
        ...
        <StyledScanLink
          useBscCoinFallback
          href={getBlockExploreLink(block, "block")}
          ml="8px"
        >
          {block}
        </StyledScanLink>
