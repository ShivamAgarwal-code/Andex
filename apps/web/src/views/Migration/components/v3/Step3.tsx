...
import {
  FEE_AMOUNT_DETAIL,
  SelectContainer,
} from "views/AddLiquidityV3/formViews/V3FormView/components/shared";
...
export function Step3() {
  ...
  const [token0, token1] =
    chainId && MOCK_TOKENS[chainId]
      ? MOCK_TOKENS[chainId]
      : MOCK_TOKENS[ChainId.MODE_MAINNET];
  ...
  const leftRangeTypedValue =
    formState.leftRangeTypedValue || tryParsePrice(token0, token1, "0.75");
  const rightRangeTypedValue =
    formState.rightRangeTypedValue || tryParsePrice(token0, token1, "1.25");
    ...
    (domain: [number, number], mode: string | undefined) => {
      ...
      batch(() => {
        ...
        if (
          ...
          onLeftRangeInput(
            tryParsePrice(token0, token1, leftRangeValue.toString())
          );
        }
        ...
        if (
          ...
          if (rightRangeValue < 1e35) {
            onRightRangeInput(
              tryParsePrice(token0, token1, rightRangeValue.toString())
            );
          }
        }
      });
  ...
  return (
    <AtomBox textAlign="center">
      <Heading scale="lg" pb="16px">
        {t("Quick Start")} - {t("Andex V3")}
      </Heading>
      <Text pb="48px">
        {t(
          "In Andex Exchange V3, liquidity providers are able to customize the trading fee tier and concentrate their liquidity to a specific price range to maximize their capital efficiency."
        )}
      </Text>
      ...
      >
        ...
        <LightCard minWidth={["100%", null, null, "50%"]} p="32px">
          <AutoRow gap="24px" justifyContent="center" py="12px">
            ...
            <AutoRow gap="12px" width="100%" paddingX="24px">
              <AtomBox position="relative" width="100%">
                <Chart
                  ...
                  data={{
                    current: MOCK.price,
                    series: MOCK.formattedData as any[],
                  }}
            ...
            <Text color="textSubtle">
              {t(
                "Your liquidity is only active and earning fees when the current price stays within the price range."
              )}
            </Text>
            <Text color="textSubtle">
              {t(
