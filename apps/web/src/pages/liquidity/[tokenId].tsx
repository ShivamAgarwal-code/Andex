...
export default function PoolPage() {
  ...
  const parsedTokenId = tokenIdFromUrl
    ? BigInt(tokenIdFromUrl as string)
    : undefined;
    ...
    <>
      <LightGreyCard mb="16px">
        <AutoRow justifyContent="space-between" mb="8px">
          ...
          <Text>
            {feeValueUpper
              ? formatCurrencyAmount(feeValueUpper, 4, locale)
              : "-"}
          </Text>
        </AutoRow>
        <AutoRow justifyContent="space-between">
          ...
          <Text>
            {feeValueLower
              ? formatCurrencyAmount(feeValueLower, 4, locale)
              : "-"}
          </Text>
        </AutoRow>
      </LightGreyCard>
  ...
  return (
    <Page>
      {!isLoading && (
        <NextSeo
          title={`${currencyQuote?.symbol}-${currencyBase?.symbol} V3 LP #${tokenIdFromUrl}`}
        />
      )}
      <BodyWrapper>
        {isLoading ? (
            ...
            <AppHeader
              title={
                <Box
                  ...
                  width="100%"
                  style={{ flex: 1 }}
                  minWidth={["auto", "auto", "max-content"]}
                >
                    ...
                    >
                      <AutoRow justifyContent="space-between" mb="8px">
                        <Flex>
                          ...
                          >
                            {
                              unwrappedToken(positionValueUpper?.currency)
                                ?.symbol
                            }
                          </Text>
                        </Flex>
                        <Flex justifyContent="center">
                          <Text small mr="4px">
                            <FormattedCurrencyAmount
                              currencyAmount={positionValueUpper}
                            />
                          </Text>
                        </Flex>
                      ...
                      <AutoRow justifyContent="space-between">
                        <Flex>
                          ...
                          >
                            {
                              unwrappedToken(positionValueLower?.currency)
                                ?.symbol
                            }
                          </Text>
                        </Flex>
                        <Flex justifyContent="center">
                          <Text small mr="4px">
                            <FormattedCurrencyAmount
                              currencyAmount={positionValueLower}
                            />
                          </Text>
                        </Flex>
                        <RowBetween justifyContent="flex-end">
                ...
                <Flex mb="8px">
                  <Flex ml="auto" alignItems="center">
                    ...
                    <Toggle
                      ...
                      onChange={() =>
                        setReceiveWNATIVE((prevState) => !prevState)
                      }
                    />
                  </Flex>
                </Flex>
                  ...
                  notEnoughLiquidity={Boolean(
                    fiatValueOfLiquidity
                      ? fiatValueOfLiquidity.lessThan(
                          ...
                          new Fraction(
                            BigInt(20) *
                              fiatValueOfLiquidity.decimalScale *
        ...
        gql`
          query positionHistory($tokenId: String!) {
            positionSnapshots(
              ...
              transaction {
                mints(
                  where: { or: [{ amount0_gt: "0" }, { amount1_gt: "0" }] }
                ) {
                  id
                  timestamp
                  amount1
                  ...
                  .map((collectTx) => {
                    ...
                    if (foundSameTxBurn) {
                      ...
                      return {
                        ...collectTx,
                        amount0: String(
                          +collectTx.amount0 - +foundSameTxBurn.amount0
                        ),
                        amount1: String(
                          +collectTx.amount1 - +foundSameTxBurn.amount1
                        ),
                      };
                    }
                    return collectTx;
...
function PositionHistoryRow({
  ...
  const date = useMemo(
    () => dayjs.unix(+positionTx.timestamp),
    [positionTx.timestamp]
  );
  const mobileDate = useMemo(
    () => isMobile && date.format("YYYY/MM/DD"),
    [isMobile, date]
  );
  const mobileTime = useMemo(
    () => isMobile && date.format("HH:mm:ss"),
    [isMobile, date]
  );
  const desktopDate = useMemo(
    () => !isMobile && date.toDate().toLocaleString(),
    [isMobile, date]
  );
    ...
    return (
      <Box>
        <AutoRow>
          <ScanLink
            useBscCoinFallback={
              chainId ? ChainLinkSupportChains.includes(chainId) : false
            }
