...
export function UnsupportedNetworkModal({
  ...
  const chainId = useLocalNetworkChain() || ChainId.MODE_MAINNET;
  ...
  const supportedMainnetChains = useMemo(
    () =>
      chains.filter(
        (chain) => !chain.testnet && pageSupportedChains?.includes(chain.id)
      ),
      ...
      <Grid style={{ gap: "16px" }} maxWidth="336px">
          ...
          <Button
            ...
            onClick={() => {
              if (supportedMainnetChains.map((c) => c.id).includes(chainId)) {
                ...
                switchNetworkAsync(ChainId.MODE_MAINNET);
              }
            }}
          >
            {isLoading ? (
              <Dots>{t("Switch network in wallet")}</Dots>
            ) : (
        ...
        {isConnected && (
          <Button
            ...
            onClick={() =>
              logout().then(() => {
                switchNetworkLocal(ChainId.MODE_MAINNET);
              })
            }
          >
            {t("Disconnect Wallet")}
