...
const PageNetworkSupportModal = dynamic(
  () =>
    import("./PageNetworkSupportModal").then(
      (mod) => mod.PageNetworkSupportModal
    ),
  { ssr: false }
);
const WrongNetworkModal = dynamic(
  () => import("./WrongNetworkModal").then((mod) => mod.WrongNetworkModal),
  {
    ssr: false,
  }
);
const UnsupportedNetworkModal = dynamic(
  () =>
    import("./UnsupportedNetworkModal").then(
      (mod) => mod.UnsupportedNetworkModal
    ),
  { ssr: false }
);
...
export const NetworkModal = ({
  ...
  const [dismissWrongNetwork, setDismissWrongNetwork] = useAtom(
    hideWrongNetworkModalAtom
  );

  const isBNBOnlyPage = useMemo(() => {
    return (
      pageSupportedChains?.length === 1 &&
      pageSupportedChains[0] === ChainId.MODE_MAINNET
    );
  }, [pageSupportedChains]);

  const isPageNotSupported = useMemo(
    () =>
      Boolean(pageSupportedChains.length) &&
      !pageSupportedChains.includes(chainId),
    ...
    return (
      <ModalV2 isOpen closeOnOverlayClick={false}>
        <UnsupportedNetworkModal
          pageSupportedChains={
            pageSupportedChains?.length ? pageSupportedChains : CHAIN_IDS
          }
        />
      </ModalV2>
    );
