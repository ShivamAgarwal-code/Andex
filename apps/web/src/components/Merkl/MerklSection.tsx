...
export function MerklSection({
  ...
  const { claimTokenReward, isClaiming, rewardsPerToken, hasMerkl } =
    useMerkl(poolAddress);

  if (
    !rewardsPerToken.length ||
    (!hasMerkl && rewardsPerToken.every((r) => r.equalTo("0")))
  )
    return null;
      ...
      {isStakedInMCv3 ? (
        <Message variant="warning">
          <MessageText color="textSubtle">
            To earn rewards on Merkl, unstake this position from Andex Farms.
            <br />
            {learnMoreComp}
          </MessageText>
        </Message>
        ...
        <Message variant="warning">
          <MessageText color="textSubtle">
            This Merkl campaign is NOT rewarding out-of-range liquidity. To earn
            rewards, adjust your position.
            <br />
            {learnMoreComp}
          </MessageText>
        </Message>
        ...
        <Message variant={notEnoughLiquidity ? "warning" : "primary"}>
          <MessageText color={notEnoughLiquidity ? "textSubtle" : ""}>
            ...
            <Link
              ...
              color="currentColor"
              style={{ display: "inline-flex" }}
              href="https://merkl.angle.money/?times=active%2Cfuture%2C&phrase=Andex"
            >
              {t("here")}
            </Link>{" "}
            <br />
