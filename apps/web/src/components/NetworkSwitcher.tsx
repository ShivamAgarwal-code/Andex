...
const WrongNetworkSelect = ({ switchNetwork, chainId }) => {
  ...
  const localChainId = useLocalNetworkChain() || ChainId.MODE_MAINNET;
  const [, setSessionChainId] = useSessionChainId();

  const localChainName =
    chains.find((c) => c.id === localChainId)?.name ?? "MODE";
...
const SHORT_SYMBOL = {
  ...
  [ChainId.MODE_TESTNET]: "tMODE",
} as const satisfies Record<ChainId, string>;

export const NetworkSwitcher = () => {
  const { t } = useTranslation();
  const { chainId, isWrongNetwork, isNotMatched } = useActiveChainId();
  const { pendingChainId, isLoading, canSwitch, switchNetworkAsync } =
    useSwitchNetwork();
  ...
  const foundChain = useMemo(
    () =>
      chains.find(
        (c) => c.id === (isLoading ? pendingChainId || chainId : chainId)
      ),
    [isLoading, pendingChainId, chainId]
  );
  const symbol =
    (foundChain?.id
      ? SHORT_SYMBOL[foundChain.id] ?? NATIVE[foundChain.id]?.symbol
