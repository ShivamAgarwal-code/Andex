...
export const V3SwapPromotionIcon: React.FC<{
  wrapperStyle?: React.CSSProperties;
}> = ({ wrapperStyle }) => {
  const { t } = useTranslation();
  const [mobileCampaignTooltipShow, setMobileCampaignTooltipShow] =
    useState(false);
  useEffect(() => {
    setTimeout(() => {
  ...
  } = useTooltip(<Text>{t("Claim $135K ANDX Airdrop & Exclusive NFT")}</Text>, {
    placement: "top",
    trigger: "hover",
