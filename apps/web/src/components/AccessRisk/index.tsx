...
export function useTokenRisk(token?: Token) {
  return useSWRImmutable(
    token &&
      token.address &&
      token.chainId === ChainId.MODE_MAINNET && [
        "risk",
        token.chainId,
        token.address,
      ],
...
const AccessRiskComponent: React.FC<AccessRiskProps> = ({ token }) => {
  ...
  const tokenInLists = useMemo(() => {
    ...
    return tokenLists.some((tokenInfoList) => {
      return tokenInfoList?.some(
        (tokenInfo) => tokenInfo.address === token.address
      );
    });
  }, [lists, token?.address]);
  ...
  const isDataLoading = useMemo(
    () => !data || (data?.riskLevel === TOKEN_RISK.UNKNOWN && !data?.hasResult),
    [data]
  );
  ...
  const tagColor = useMemo(() => {
    ...
    if (
      data.riskLevel >= TOKEN_RISK.LOW &&
      data.riskLevel <= TOKEN_RISK.MEDIUM
    ) {
      return "warning";
    }
    if (
      data.riskLevel >= TOKEN_RISK.VERY_LOW &&
      data.riskLevel <= TOKEN_RISK.SOME_RISK
    ) {
      return "primary";
    }
    return "textDisabled";
