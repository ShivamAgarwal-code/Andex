...
import {
  cakePoolBalanceStrategy,
  createTotalStrategy,
} from "views/Voting/strategies";
...
const useCakeBenefits = () => {
  ...
  const currentBscBlock = useChainCurrentBlock(ChainId.MODE_MAINNET);

  const { data, status } = useSWR(
    account && currentBscBlock && ["cakeBenefits", account],
    async () => {
      const [userInfo, currentPerformanceFee, currentOverdueFee, sharePrice] =
      ...
      const currentOverdueFeeAsBigNumber = new BigNumber(
        currentOverdueFee.toString()
      );
      ...
      const userSharesAsBignumber = new BigNumber(
        userContractResponse.shares.toString()
      );
      ...
      const lockedCake = [VaultPosition.None, VaultPosition.Flexible].includes(
        lockPosition
      )
        ? "0.00"
        : convertSharesToCake(
          ...
          ).cakeAsNumberBalance.toLocaleString("en", {
      ...
      if (lockPosition === VaultPosition.Locked) {
        // @ts-ignore
        // TODO: Fix viem
        const credit = await ifoCreditAddressContract.read.getUserCredit([
          account,
        ]);
        iCake = getBalanceNumber(
          new BigNumber(credit.toString())
        ...
        const [cakeVaultBalance, total] = await getScores(
          PANANDX_SPACE,
          [
            cakePoolBalanceStrategy("v1"),
            createTotalStrategy(poolAddresses, "v1"),
          ],
          ChainId.MODE_MAINNET.toString(),
          [account],
          Number(currentBscBlock)
        );
        vCake = {
          vaultScore: cakeVaultBalance[account]
            ? cakeVaultBalance[account].toLocaleString("en", {
                maximumFractionDigits: 3,
              })
            : "0",
          totalScore: total[account]
            ? total[account].toLocaleString("en", { maximumFractionDigits: 3 })
            : "0",
        };
      }
