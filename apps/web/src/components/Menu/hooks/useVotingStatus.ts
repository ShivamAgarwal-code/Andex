...
import { PANANDX_SPACE, ADMINS } from "views/Voting/config";

export const getCoreProposal = async (
  type: ProposalState
): Promise<Proposal[]> => {
  const response = await request(
    ...
    gql`
      query getProposals($first: Int!, $skip: Int!, $state: String!, $admins: [String]!) {
        proposals(first: $first, skip: $skip, where: { author_in: $admins, space_in: "${PANANDX_SPACE}", state: $state }) {
          id
        }
      }
    `,
