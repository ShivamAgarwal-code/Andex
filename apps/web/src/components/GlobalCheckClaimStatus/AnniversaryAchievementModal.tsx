...
const AnniversaryAchievementModal: React.FC<AnniversaryModalProps> = ({
  excludeLocations,
}) => {
  ...
  const [showOnceAnniversaryModal, setShowOnceAnniversaryModal] =
    useShowOnceAnniversaryModal();

  const hasDisplayedModal = useRef(false);
  const [isFirstTime, setIsFirstTime] = useState(true);
  const [show, setShow] = useState(false);
  const [isLoading, setIsLoading] = useState(false);
  const [canClaimAnniversaryPoints, setCanClaimAnniversaryPoints] =
    useState(false);

  const contract = useAnniversaryAchievementContract({
  ...
  useEffect(() => {
    ...
    if (account && chainId === ChainId.MODE_MAINNET) {
      fetchClaimAnniversaryStatus();
    }
  }, [account, chainId, contract]);

  useEffect(() => {
    const matchesSomeLocations = excludeLocations.some((location) =>
  ...
  const handleClick = async () => {
    ...
    try {
      const receipt = await fetchWithCatchTxError(() =>
        contract.write.claimAnniversaryPoints({ account, chainId })
      );
