...
export const fetchPublicVaultData = async ({
  ...
  try {
    ...
    const [sharePrice, shares, totalLockedAmount, totalCakeInVault] =
      await client.multicall({
        contracts: [
          ...
          {
            abi: balanceOfAbi,
            address: ANDX[ChainId.MODE_MAINNET].address,
    ...
    const totalSharesAsBigNumber =
      shares.status === "success" && shares.result
        ? new BigNumber(shares.result.toString())
        : BIG_ZERO;
    ...
    const sharePriceAsBigNumber =
      sharePrice.status === "success" && sharePrice.result
        ? new BigNumber(sharePrice.result.toString())
        : BIG_ZERO;
    ...
    return {
      ...
      totalCakeInVault: totalCakeInVault.result
        ? new BigNumber(totalCakeInVault.result.toString()).toJSON()
        : "0",
    };
  } catch (error) {
    return {
  ...
  try {
    ...
    const totalSharesAsBigNumber =
      shares.status === "success"
        ...
        : BIG_ZERO;
    const sharePriceAsBigNumber =
      sharePrice.status === "success"
        ? new BigNumber(sharePrice.result.toString())
        : BIG_ZERO;
    return {
      totalShares: totalSharesAsBigNumber.toJSON(),
      pricePerFullShare: sharePriceAsBigNumber.toJSON(),
      totalCakeInVault: new BigNumber(
        (totalCakeInVault.result || "0").toString()
      ).toJSON(),
    };
  } catch (error) {
    return {
...
export const fetchVaultFees = async ({
  ...
  try {
    ...
    const [performanceFee, withdrawalFee, withdrawalFeePeriod] =
      await client.multicall({
        contracts: [
          {
            abi: cakeVaultV2ABI,
