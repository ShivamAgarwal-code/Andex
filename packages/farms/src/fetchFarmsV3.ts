...
export async function farmV3FetchFarms({
  ...
  const lmPoolInfos = await fetchLmPools(
    v3PoolData
      .map((v3Pool) => (v3Pool[1] ? v3Pool[1] : null))
    ...
    .map((farm, index) => {
      ...
      return {
        ...
        _rewardGrowthGlobalX128:
          lmPoolInfos[lmPoolAddress].rewardGrowthGlobalX128,
...
export async function fetchMasterChefV3Data({
  ...
  const [poolLength, totalAllocPoint, latestPeriodCakePerSecond] =
    await provider({ chainId }).multicall({
      contracts: [
        {
...
const getFarmAllocation = ({
  ...
  const _allocPoint =
    typeof allocPoint !== "undefined"
      ? new BN(allocPoint.toString())
      : BIG_ZERO;
  const poolWeight =
    !!totalAllocPoint && !_allocPoint.isZero()
      ? _allocPoint.div(totalAllocPoint.toString())
      : BIG_ZERO;

  return {
    poolWeight: poolWeight.toString(),
    multiplier: !_allocPoint.isZero()
      ? `${+_allocPoint.div(10).toString()}X`
      : `0X`,
  };
};
...
export const fetchCommonTokenUSDValue = async (
  priceHelper?: PriceHelper
): Promise<CommonPrice> => {
  return fetchTokenUSDValues(priceHelper?.list || []);
};

export const fetchTokenUSDValues = async (
  currencies: Currency[] = []
): Promise<CommonPrice> => {
  ...
  if (currencies.length > 0) {
    const list = currencies
      .map(
        (currency) =>
          `${getLlamaChainName(currency.chainId)}:${currency.wrapped.address}`
      )
      .join(",");
...
export function getFarmsPrices(
  ...
  const commonPriceFarms = farms.map((farm) => {
    ...
    if (
      ...
      farm.token.chainId in ANDX &&
      farm.token.equals(ANDX[farm.token.chainId as keyof typeof ANDX])
    ) {
      tokenPriceBusd = new BN(cakePriceUSD);
    }
    if (
      quoteTokenPriceBusd.isZero() &&
      farm.quoteToken.chainId in ANDX &&
      farm.quoteToken.equals(ANDX[farm.quoteToken.chainId as keyof typeof ANDX])
    ) {
    ...
    if (tokenPriceBusd.isZero()) {
      ...
      if (quoteTokenPriceBusd.isZero()) {
        ...
        if (ifQuoteTokenPriceFound) {
          quoteTokenPriceBusd = farm.quoteToken.equals(
            ifQuoteTokenPriceFound.token
          )
