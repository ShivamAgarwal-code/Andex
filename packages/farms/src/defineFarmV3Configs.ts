...
import { ANDX, unwrappedToken } from "@pancakeswap/tokens";
...
function sortFarmLP(token0: Token, token1: Token) {
  const commonTokens =
    priceHelperTokens[token0.chainId as keyof typeof priceHelperTokens];
  if (commonTokens) {
    const commonTokensList = [
      WNATIVE[token0.chainId as keyof typeof WNATIVE],
      ...commonTokens.list,
      ANDX[token0.chainId as keyof typeof ANDX]
        ? ANDX[token0.chainId as keyof typeof ANDX]
        : undefined,
    ...
    if (someToken0 && someToken1) {
      return commonTokensList.indexOf(token0) > commonTokensList.indexOf(token1)
        ? [token0, token1]
        : [token1, token0];
    }
    if (someToken0) {
...
export function defineFarmV3Configs(
  farmConfig: FarmConfigV3[]
  ...
  return farmConfig.map((config) => {
    ...
    return {
      ...
      token,
      quoteToken,
      lpSymbol: `${unwrappedToken(token)?.symbol}-${unwrappedToken(quoteToken)
        ?.symbol} LP`,
    };
  });
}
